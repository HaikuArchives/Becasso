...
: BWindow (frame, name, B_DOCUMENT_WINDOW, B_WILL_ACCEPT_FIRST_CLICK)
{
	...
	hFrame.Set (0, viewFrame.bottom + 1, viewFrame.right + 1,	
		viewFrame.bottom + B_H_SCROLL_BAR_HEIGHT + 1);
	h = new BScrollBar (hFrame, NULL, layerView, 0, 0, B_HORIZONTAL);
	vFrame.Set (viewFrame.right + 1, viewFrame.top, 
		viewFrame.right + B_V_SCROLL_BAR_WIDTH + 1, viewFrame.bottom + 1);
...
void LayerWindow::doChanges (int index)
{
	if (index == -1)
	{
...
void LayerWindow::Quit ()
{
	myWindow->PostMessage ('layQ');
	inherited::Quit();
}

void LayerWindow::WindowActivated (bool /* active */)
{
//	if (active)
//		doChanges (-1);
}