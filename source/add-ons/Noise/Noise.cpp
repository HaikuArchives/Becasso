...
public:
			 NoiseView (BRect rect) : BView (rect, "noise_view", B_FOLLOW_NONE, B_WILL_DRAW)
			 {
			 	...
			 	gType = N_ADDITIVE;
				ResizeTo (188, 82);
				Slider *xSlid = new Slider (BRect (8, 62, 180, 78), 50, "Sigma", 1, MAXAMOUNT, 1, new BMessage ('nSch'));
				AddChild (xSlid);
				BBox *type = new BBox (BRect (4, 4, 184, 58), "type");
				type->SetLabel ("Type");
				AddChild (type);
				BRadioButton *tAdd = new BRadioButton (BRect (8, 13, 164, 30), "add", "Additive", new BMessage ('nTad'));
				BRadioButton *tMul = new BRadioButton (BRect (8, 30, 164, 47), "mul", "Multiplicative", new BMessage ('nTmp'));
				type->AddChild (tAdd);
				type->AddChild (tMul);
				tAdd->SetValue (true);
				xSlid->SetValue (5);
...
void NoiseView::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case 'nSch':
		...
		else
		{
			...
			case N_MULTIPLICATIVE:
			{
				for (uint32 y = 0; y < h; y++)
				{
					...
					for (uint32 x = 0; x < w; x++)
					{
						*(++destline) = clipchar (*(++srcline)*(255 + xvec[random (MAXRANDSIZE)])/255);
					}
				}
				break;
			}
