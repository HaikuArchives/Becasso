...
public:
			 MotionBlurView (BRect rect) : BView (rect, "motion_blur_view", B_FOLLOW_ALL, B_WILL_DRAW)
			 {
			 	...
			 	gAngle = 0;
				ResizeTo (188, 48);
				Slider *sSlid = new Slider (BRect (8, 8, 180, 24), 60, "Distance", 2, MAXSIZE, 1, new BMessage ('mblS'));
				Slider *aSlid = new Slider (BRect (8, 28, 180, 44), 60, "Angle", 0, 360, 1, new BMessage ('mblA'));
				AddChild (sSlid);
				AddChild (aSlid);
				sSlid->SetValue (5);
				aSlid->SetValue (0);
...
void MotionBlurView::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case 'mblS':
	...
	case M_DRAW:
	{
		...
		for (y = 0; y < h; y++)
		{
			...
			for (x = 0; x < w; x++)
			{
				...
				{
					n = int (distance*(inSelection ? *mapbits : 255)/255); /* This effectively rounds it down */
					if (n > 1)
					{
						px = int (n*cangle);
						py = int (n*sangle);
