...
{
public:
			 CaptureWindow (BRect rect) : BWindow (rect, "PalmCam", B_TITLED_WINDOW, B_NOT_ZOOMABLE | B_WILL_ACCEPT_FIRST_CLICK)
			 {
			 	fBPS = B_9600_BPS;
			 	fPort[0] = 0;
			 	abort = false;
...
status_t reopen_camera (const char *port, data_rate bps);
status_t update_list ();

void CaptureWindow::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case 'bps1':
...
status_t addon_init (uint32 index, becasso_addon_info *info)
{
	...
	window->fList = new BListView (BRect (110, 8, 294 - B_V_SCROLL_BAR_WIDTH, 170), "image list", B_MULTIPLE_SELECTION_LIST, B_FOLLOW_ALL_SIDES);
	window->fList->SetSelectionMessage (new BMessage ('PCsc'));

	BScrollView *scroller = new BScrollView ("scroller", window->fList, B_FOLLOW_ALL_SIDES, 0, false, true);
...
BBitmap *bitmap (char *title)
{
	...
	BBitmap *b = NULL;
	if (BTranslatorRoster::Default()->Translate (&inStream, NULL, NULL, &outStream, B_TRANSLATOR_BITMAP) < B_OK)
	{
