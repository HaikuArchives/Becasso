...
const char addonInfo[] =
	"Dither a 32-bit bitmap to 8-bit. By Edmund Vermeulen <edmundv@xs4all.nl>.";
...
)
{
	...
	for (float y = 0.0; y <= height; ++y) {
		...
		uchar* src = static_cast<uchar*>(sourceBitmap->AccessBits(
			BRect(
				source_bounds.left, source_bounds.top + y, source_bounds.right,
				source_bounds.top + y
			),
			&source_row_bytes
		));
		...
		uchar* in_bitmap =
			static_cast<uchar*>(destBitmap->AccessBits(BRect(0.0, y, width, y), &dest_row_bytes));
		...
		for (float x = 0.0; x <= width; ++x) {
			...
			int32 r = int32(src[2]) + int32(
										  r_delta * 0.4375 + deltas[0] * 0.0625 +
										  deltas[3] * 0.3125 + deltas[6] * 0.1875
									  );
			int32 g = int32(src[1]) + int32(
										  g_delta * 0.4375 + deltas[1] * 0.0625 +
										  deltas[4] * 0.3125 + deltas[7] * 0.1875
									  );
			int32 b = int32(src[0]) + int32(
										  b_delta * 0.4375 + deltas[2] * 0.0625 +
										  deltas[5] * 0.3125 + deltas[8] * 0.1875
									  );
