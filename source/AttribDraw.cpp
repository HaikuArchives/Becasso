...
: AttribView (BRect (0, 0, 164, 58), lstring (20, "Draw"))
{
	...
	BMenuField *dMode = new BMenuField (BRect (8, 6, 156, 24), "dMode", lstring (329, "Drawing Mode:"), fModePU);
	dMode->SetDivider (82);
	...
	if (BuiltInTablet)
	{
		...
		BMenuField *dTablet = new BMenuField (BRect (8, 30, 156, 48), "dTablet", lstring (330, "Tablet: "), fTabletPU);
...
status_t AttribDraw::GetSupportedSuites (BMessage *message)
{
...
BHandler *AttribDraw::ResolveSpecifier (BMessage *message, int32 index, BMessage *specifier, int32 command, const char *property)
{
	return inherited::ResolveSpecifier (message, index, specifier, command, property);
}

void AttribDraw::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case 'TBL1':
