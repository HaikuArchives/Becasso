...
{
public:
			 	 CaptureWindow (BRect rect) : BWindow (rect, "Video Grabber", B_TITLED_WINDOW, B_NOT_ZOOMABLE | B_WILL_ACCEPT_FIRST_CLICK) {};
virtual			~CaptureWindow ();
virtual bool	 QuitRequested () { Hide(); TearDownNodes(); return false; };
...
void ErrorAlert(const char * message, status_t err);

CaptureWindow::~CaptureWindow ()
{
}

BBitmap *CaptureWindow::Grab()
{
...
void CaptureWindow::TearDownNodes ()
{
	if (!fMediaRoster)
		return;
	if (fVideoConsumer)
...
status_t addon_init (uint32 index, becasso_addon_info *info)
{
	...
	window->fVideoView = new BView (BRect (4, 4, 4 + 159, 4 + 119), "video", B_FOLLOW_ALL, B_WILL_DRAW);
