...
: BWindow (frame, title, B_TITLED_WINDOW, B_NOT_RESIZABLE | B_NOT_ZOOMABLE | B_WILL_ACCEPT_FIRST_CLICK)
{
	current = -1;
	numviews = 0;
...
int AttribWindow::AddView (AttribView *view, BBitmap *icon)
{
	view_n_icon entry;
	entry.view = view;
	entry.icon = 0;
	...
	{
		view->MoveBy (40, 0);
		BitmapView *iview = new BitmapView (BRect (0, 0, 39, view->Bounds().Height()), "icon", icon, B_OP_OVER, false);
...
void AttribWindow::RaiseView (int index)
{
	Lock();
	if (current != -1)
	{
...
bool AttribWindow::QuitRequested ()
{
	Hide();
	return false;
}

void AttribWindow::Show ()
{
	inherited::Show();
}

void AttribWindow::Hide ()
{
	inherited::Hide();
}

BHandler *AttribWindow::ResolveSpecifier (BMessage *message, int32 index, BMessage *specifier, int32 command, const char *property)
{
...
void AttribWindow::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	default:
