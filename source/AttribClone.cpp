...
void AttribClone::ConstructBrush ()
{
	delete fBrush;
...
inline float hfunc (float r, float s)
{
	return (pow (r, s));
}

void AttribClone::mkGaussianBrush (Brush *b, float sigmaxsq, float sigmaysq, float angle, int cval, float hardness)
{
	int hw = b->Width()/2;
	int hh = b->Height()/2;
...
void AttribClone::mkDiagonalBrush (Brush *b, int dir)
{
...
status_t AttribClone::GetSupportedSuites (BMessage *message)
{
...
BHandler *AttribClone::ResolveSpecifier (BMessage *message, int32 index, BMessage *specifier, int32 command, const char *property)
{
//	printf ("\nmessage:\n");
...
void AttribClone::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case B_GET_PROPERTY:
