...
CKView::CKView (BRect rect)
: BView (rect, "CKView", B_FOLLOW_ALL, B_WILL_DRAW)
{
 	gSigma = 5;
 	gThreshold = 5;
 	gMode = CK_BLUE;
	...
	BRadioButton *rbBl = new BRadioButton (BRect (4, 14, 170, 30), "blue", "Blue", new BMessage ('ckBl'));
	BRadioButton *rbFG = new BRadioButton (BRect (4, 32, 170, 48), "fg", "Foreground", new BMessage ('ckFG'));
	BRadioButton *rbBG = new BRadioButton (BRect (4, 50, 170, 66), "bg", "Background", new BMessage ('ckBG'));
	mode->AddChild (rbBl);
	mode->AddChild (rbFG);
	mode->AddChild (rbBG);
	...
	Slider *ySlid = new Slider (BRect (8, 102, 180, 118), 60, "Threshold", 0, 100, 1, new BMessage ('ckTh'));
	AddChild (xSlid);
	AddChild (ySlid);
...
void CKView::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case 'ckSg':
