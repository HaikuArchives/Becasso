...
: AttribView (BRect (0, 0, 148, 54), lstring (28, "Lines"))
{
	...
	AddChild (lSlid);
	fPenSize = 1;
	lFC = new BCheckBox (BRect (8, 30, 120, 48), "lFC", lstring (348, "Fill Corners"), new BMessage ('ALfc'));
	lFC->SetValue (1);
	fFillCorners = true;
	AddChild (lFC);
	fCurrentProperty = 0;
...
status_t AttribLines::GetSupportedSuites (BMessage *message)
{
...
BHandler *AttribLines::ResolveSpecifier (BMessage *message, int32 index, BMessage *specifier, int32 command, const char *property)
{
...
void AttribLines::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case B_GET_PROPERTY:
