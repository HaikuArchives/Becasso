...
: BView (frame, name, B_FOLLOW_LEFT | B_FOLLOW_TOP, B_WILL_DRAW | B_FRAME_EVENTS)
{
	...
	hide = new BCheckBox (BRect (fThumbHSize + 8, 2, fThumbHSize + 50, 24), "hide", lstring (164, "Hide"), msg);
	AddChild (hide);
	...
	msg->AddInt32 ("index", index);
	Slider *ga = new Slider (BRect (fThumbHSize + 54, 4, LAYERITEMWIDTH - 4, 20), 0, "", 0, 255, 1, msg, B_HORIZONTAL);
	...
	item->SetTarget (fMyView->Window());
	fModePU->AddItem (item);
	BMenuField *mode = new BMenuField (BRect (fThumbHSize + 8, 22, fThumbHSize + 140, 34), "mode", lstring (167, "Operation:"), fModePU);
...
void LayerItem::Refresh (Layer *layer)
{
	fLayer = layer;
	DrawThumbOnly();
}

void LayerItem::MouseMoved (BPoint point, uint32 transit, const BMessage *message)
{
...
void LayerItem::select (bool sel)
{
...
void LayerItem::MouseDown (BPoint point)
{
...
void LayerItem::Draw (BRect /* update */)
...
//        cache the thumbnail bitmaps.  However, this works and updates in realtime.
{
	...
	for (int i = 0; i < thumbnail->BitsLength()/4; i++)
	{
		...
		register uint32 b = (((pixel & 0xFF000000) >> 8)*a - (a << 24) + (255 << 24)) & 0xFF000000;
		register uint32 g = (((pixel & 0x00FF0000) >> 8)*a - (a << 16) + (255 << 16)) & 0x00FF0000;
		...
		*bits = (b | g | r | a);
		...
		register uint32 r = ((((pixel & 0x00FF0000)*a) >> 8) - (a << 16) + (255 << 16)) & 0x00FF0000;
		*bits = (b | g | r | (a << 24));
#endif
	}
	...
	DrawString (fLayer->getName(), BPoint (thumbRect.right + 8, thumbRect.bottom - 4));	// Should be dynamic...
	StrokeRect (Bounds());
	Sync();
...
void LayerItem::MessageReceived (BMessage *message)
{
