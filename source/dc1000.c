...
static const char	envport[]	= "DSCPORT",
			envbaud[]	= "DSCBAUD";

const speed_t		defaultbaud	= B9600;

#if __BEOS__
static const char	attr[]		= "BEOS:TYPE",
			mime[]		= "image/jpeg";
#endif
...
int	list(dsc_t*);
int	listlong(dsc_t*);
int	get(dsc_t*, int*, const char*, int, int, int);
int	getall(dsc_t*, int, int, int);
int     thumbnail(dsc_t*, int*, const char*, int, int);
int     thumbnailall(dsc_t*, int, int);
int	show(dsc_t*, int*, int);
int	delete(dsc_t*, int*, int);
int	parse(int*, const char*);
int	getimage(dsc_t*, int, const char*, int, int, int);
...
int main(int argc, char *argv[])
{
	...
	for (;;)
	{
		...
		static const struct option longopts[] =
		{
			...
			{ "listlong", no_argument, NULL, 'L' },
			{ "get", required_argument, NULL, 'g' },
			{ "getall", no_argument, NULL, 'G' },
			{ "thumbnail", required_argument, NULL, 't' },
			{ "thumbnailall", no_argument, NULL, 'T' },
...
"\n", argv[0]
			...
			case 'v': /* version */
				printf(
"dc1000 0.5 (2000-06-02) BeOS beta\n"
...
"copyright notice is left intact.\n"
	...
	if (output != NULL && op != 'g')
		fprintf(
			stderr,
			"%s: Ignoring option -o when using -%c\n",
			logname, op
		);
	...
	if (sigaction(SIGINT, &sigact, NULL) == -1)
		fprintf(
			stderr,
			"%s: System call sigaction() failed\n",
			logname
		);
#endif
...
int get(
	dsc_t *dsc, int *index, const char *output,
	int flag_V, int flag_q, int flag_c
)
{
	int	n;
	...
	if (output != NULL && index[0] > 1)
	{
		...
		fprintf(
			stderr,
			"%s: Ignoring option -o for multiple images\n",
			logname
		);
	}
	...
	for (n = 1; n <= DSCMAXIMAGE; n ++)
		if (	index[n] &&
			getimage(dsc, n, output, flag_V, flag_q, flag_c) == -1)
			return -1;
...
int getall(dsc_t *dsc, int flag_V, int flag_q, int flag_c)
{
	int		n, nimg;
	dsc_quality_t	buf[DSCMAXIMAGE];
...
)
{
	...
	for (n = 1; n <= DSCMAXIMAGE; n ++)
		if (	index[n] &&
			getimage(dsc, -n, output, flag_V, flag_q, 0) == -1)
			return -1;
...
int thumbnailall(dsc_t *dsc, int flag_V, int flag_q)
{
	int		n, nimg;
	dsc_quality_t	buf[DSCMAXIMAGE];
...
int show(dsc_t *dsc, int *index, int flag_V)
{
	int	n;
...
int parse(int *index, const char *arg)
{
	int	i, p, lp, l, n;
	char	*a, ap;
...
int getimage(
	dsc_t *dsc, int index, const char *name,
	int flag_V, int flag_q, int flag_c
)
{
#ifdef DEBUG
...
# define returnerror() { errline = __LINE__; goto getimage_ERROR; }
# define returnerrno() { \
	errline = __LINE__; \
...
# define returnerror() { goto getimage_ERROR; }
# define returnerrno() { \
	dsc->lasterror.lerror = EDSCSERRNO; \
	dsc->lasterror.lerrno = errno; \
	goto getimage_ERROR; }
	...
	if (flag_c)
	{
		if (stat(name, &statbuf) == -1)
		{
			...
			{
				printf(
					"Remaining size is %u bytes. Reading",
					(unsigned int)(imagesize - csize)
				);
				if (flag_q)
					printf("...\n");
			} else if (! flag_q) {
	...
	if (file != STDOUT_FILENO)
	{
		...
		if (fs_write_attr(
			file, attr, B_STRING_TYPE, 0,
			mime, sizeof(mime)) != sizeof(mime))
		{
# ifdef DEBUG
			fprintf(
				stderr,
...
getimage_ERROR:
	...
	fprintf(
		...
		errline,
		dsc->lasterror.lerror == EDSCSERRNO ?
		dsc->lasterror.lerrno :
		dsc->lasterror.lerror
	);
#else
	if (! flag_q)
		putchar('\n');
