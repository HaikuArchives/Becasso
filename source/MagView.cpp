...
bool
MagView::contains(BPoint p) const
{
...
MagView::Draw(BRect update)
{
	...
	if (grid_visible) {
		...
		if (grid_visible == 1)
			...
			Hi = White;
		BeginLineArray((update.right - update.left + update.bottom - update.top) / zoom + 8);
		for (long x = long(update.left / zoom - 1); x <= long(update.right / zoom + 1); x++)
			AddLine(BPoint(x * zoom, update.top), BPoint(x * zoom, update.bottom), Hi);
		for (long y = long(update.top / zoom - 1); y <= long(update.bottom / zoom + 1); y++)
			AddLine(BPoint(update.left, y * zoom), BPoint(update.right, y * zoom), Hi);
		EndLineArray();
	}
	Sync();
	delete canvas;
...
void
MagView::MouseDown(BPoint point)
{
...
void
MagView::MouseMoved(BPoint point, uint32 transit, const BMessage* msg)
{
...
void
MagView::setScrollBars(BScrollBar* _h, BScrollBar* _v)
{
	mh = _h;
	mv = _v;
}
...
void
MagView::FrameResized(float width, float height)
{
...
void
MagView::SetupUndo(BPoint p, uint32 c)
{
