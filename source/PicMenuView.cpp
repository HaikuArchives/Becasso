...
void PicMenuView::Draw (BRect updateRect)
{
	...
	if (fPMB)
	{
		...
		{
			thisbutton.Set ((i%hnum)*bw, (i/hnum)*bw, (i%hnum + 1)*bw, (i/hnum + 1)*bw);
...
void PicMenuView::MouseMoved (BPoint point, uint32 transit, const BMessage *msg)
{
	msg = msg;
	int previndex = index;
...
BRect PicMenuView::RectForIndex (int i)
{
	float bw = Bounds().Width()/hnum;
	return BRect ((i % hnum)*bw, (i/hnum)*bw, (i % hnum + 1)*bw - 1, (i/hnum + 1)*bw - 1);
}

void PicMenuView::MouseDown (BPoint point)
{
	Window()->Lock();
