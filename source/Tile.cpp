...
virtual void	 SetBitmap (BBitmap *bitmap) { delete fBitmap; fBitmap = bitmap; Invalidate(); };
...
void TileView::Draw (BRect update)
{
...
void TileView::KeyDown (const char *bytes, int32 numbytes)
{
	if (numbytes == 1)
	{
...
TileMainView::TileMainView (BRect rect)
: BView (rect, "tile main view", B_FOLLOW_ALL, B_WILL_DRAW)
{
	ResizeTo (188, 164);
	app_info ainfo;
	...
	BMenuField *catalogMenu = new BMenuField (BRect (8, 4, 180, 30), "catalog", "Catalog:", catalogPU);
...
void TileMainView::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case 'Til-':
		if (current_index > 0)
		{
			...
			entry.GetName (name);
			if (view->Bitmap())
				sprintf (title, "%s (%i x %i)", name, int (view->Bitmap()->Bounds().Height() + 1), int (view->Bitmap()->Bounds().Width() + 1));
			else
