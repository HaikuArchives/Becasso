...
//	frame->PrintToStream();
	...
	case M_DRAW:
	{
		...
		if (!inSelection || *mapbits)
			*dbits = average4 (*sbits, *(sbits + 1), 
								*(sbits + w), *(sbits + w + 1));
		else
			*dbits = *sbits;
		// Right top
		if (!inSelection || *(mapbits + w - 1))
			*(dbits + w - 1) = average4 (*(sbits + w - 2), *(sbits + w - 1), 
										  *(sbits + 2*w - 2), *(sbits + 2*w - 1));
		else
			*(dbits + w - 1) = *(sbits + w - 1);
		// Left bottom
		if (!inSelection || *(mapbits + (h - 1)*mbpr))
			*(dbits + (h - 1)*w) = average4 (*(sbits + (h - 2)*w), *(sbits + (h - 2)*w + 1), 
											  *(sbits + (h - 1)*w), *(sbits + (h - 1)*w));
		else
			*(dbits + (h - 1)*w) = *(sbits + (h - 1)*w);
		// Right bottom
		if (!inSelection || *(mapbits + (h - 1)*mbpr + w - 1))
			*(dbits + h*w - 1) = average4 (*(sbits + (h - 1)*w - 2), *(sbits + (h - 1)*w - 1),
											*(sbits + h*w - 2), *(sbits + h*w - 1));
		else
		...
		for (uint32 x = 1; x < w - 1; x++)
		{
			if (!inSelection || *(mapbits + x))
			{
				...
				*(dbits + x) = average6 (*(sbitsx - 1), *sbitsx, *(sbitsx + 1),
										  *(sbitsxl - 1), *sbitsxl, *(sbitsxl + 1));
			}
			else
				*(dbits + x) = *(sbits + x);
			...
			if (!inSelection || *(mapbits + (h - 1)*mbpr + x))
			{
				...
				*(dbits + (h - 1)*w + x) = average6 (*(sbitsx - 1), *sbitsx, *(sbitsx + 1),
		...
		for (uint32 y = 1; y < h - 1; y++)
		{
			if (!inSelection || *(mapbits + y*mbpr))
			{
				...
				*(dbits + y*w) = average6 (*(sbitsy - w), *sbitsy, *(sbitsy + w),
											*(sbitsyr - w), *sbitsyr, *(sbitsyr + w));
			}
			else
				*(dbits + y*w) = *(sbits + y*w);
			...
			if (!inSelection || *(mapbits + y*mbpr + w - 1))
			{
				...
				*(dbits + (y + 1)*w - 1) = average6 (*(sbitsy - w), *sbitsy, *(sbitsy + w),
													  *(sbitsyl - w), *sbitsyl, *(sbitsyl + w));
			}
			else
		...
		for (uint32 y = 1; y < h - 1; y++)
		{
			...
			for (uint32 x = 1; x < w - 1; x++)
			{
				if (!inSelection || *(++mapbits))
				{
					...
					*(++dbits) = average9 (*(sbitsu - 1), *sbitsu, *(sbitsu + 1),
											*(sbits - 1), *sbits, *(sbits + 1),
	...
	case M_SELECT:
	{
		...
		else
		{
			...
			*dbits = (*sbits + *(sbits + 1) + *(sbits + mbpr) + *(sbits + mbpr + 1))/4;
			// Right top
			*(dbits + mbpr - 1) = (*(sbits + mbpr - 2) + *(sbits + mbpr - 1) + *(sbits + 2*mbpr - 2) + *(sbits + 2*mbpr - 1))/4;
			// Left bottom
			*(dbits + (h - 1)*mbpr) = (*(sbits + (h - 2)*mbpr) + *(sbits + (h - 2)*mbpr + 1) + *(sbits + (h - 1)*mbpr) + *(sbits + (h - 1)*mbpr))/4;
			// Right bottom
			*(dbits + h*mbpr - 1) = (*(sbits + (h - 1)*mbpr - 2) + *(sbits + (h - 1)*mbpr - 1) + *(sbits + h*mbpr - 2) + *(sbits + h*mbpr - 1))/4;
			...
			for (uint32 x = 1; x < w - 1; x++)
			{
				...
				*(dbits + x) = (*(sbitsx - 1) + *sbitsx + *(sbitsx + 1) + *(sbitsxl - 1) + *sbitsxl + *(sbitsxl + 1))/6;
				
				sbitsx  = sbits + (h - 1)*mbpr + x;
				grey_pixel *sbitsxu = sbitsx - mbpr;
				*(dbits + (h - 1)*mbpr + x) = (*(sbitsx - 1) + *sbitsx + *(sbitsx + 1) + *(sbitsxu - 1) + *sbitsxu + *(sbitsxu + 1))/6;
			}
			...
			for (uint32 y = 1; y < h - 1; y++)
			{
				...
				*(dbits + y*mbpr) = (*(sbitsy - mbpr) + *sbitsy + *(sbitsy + mbpr) + *(sbitsyr - mbpr) + *sbitsyr + *(sbitsyr + mbpr))/6;
		
				sbitsy  = sbits  + (y + 1)*mbpr - 1;
				grey_pixel *sbitsyl = sbitsy - 1;
				*(dbits + (y + 1)*mbpr - 1) = (*(sbitsy - mbpr) + *sbitsy + *(sbitsy + mbpr) + *(sbitsyl - mbpr) + *sbitsyl + *(sbitsyl + mbpr))/6;
			}
			...
			for (uint32 y = 1; y < h - 1; y++)
			{
				...
				for (uint32 x = 1; x < w - 1; x++)
				{
					...
					*(++dbits) = (*(sbitsu - 1) + *sbitsu + *(sbitsu + 1) +
								  *(sbits - 1) + *sbits + *(sbits + 1) +
								  *(sbitsl - 1) + *sbitsl + *(sbitsl + 1))/9;
				}
			}
		}
		break;
