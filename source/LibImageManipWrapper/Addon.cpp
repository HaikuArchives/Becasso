...
WrapperWindow::WrapperWindow (BRect rect, becasso_addon_info *info) : AddOnWindow (rect, info) {
	...
	mHeight = rect.IntegerHeight(); mWidth = rect.IntegerWidth();
	...
	Background()->AddChild(new BButton(BRect(205, mHeight, 265, mHeight + 20), "", "Preview", new BMessage(MSG_FILTER_PREVIEW)));
	mHeight += 32; mWidth = 270;
	Background()->AddChild(new BCheckBox(BRect(5, mHeight, 265, mHeight + 20), "", APPLY_TO_SELECTION, new BMessage(MSG_APPLY_TO_SELECTION))); 
	mHeight += 20;
...
void WrapperWindow::SetupFilters(BMenu* sub_menu) {
...
#endif
	{
		...
		for (int i = outCount-1; i >= 0; --i)
		{
					...
					&addonCategory, &addonVersion) == B_OK)
			{
				...
				if (i == outCount-1) PostMessage(msg);
			}
		}
		delete[] outList;
	}
...
void WrapperWindow::SetupConfigView(int32 addon, const char* name) {
	mAddonId = addon;
	RemoveView();
	int width;
	BMessage dummy;
...
void WrapperWindow::MessageReceived(BMessage* msg) {
	switch(msg->what) {
...
void WrapperWindow::SetStatus(const char* text) {
	...
	switch (mode)
	{
		...
		case M_DRAW:
		{
			if (mode == M_DRAW && *outLayer == NULL || mode == M_SELECT && *outSelection == NULL) {
				// Create bitmap accessor object
				BBitmapAccessor* src_accessor = Image_CreateBBitmapAccessor(mode == M_DRAW ? inLayer : inSelection);
				if (src_accessor == NULL) {
					error = ADDON_ABORT;;
					break;
				}
				...
				delete dest_accessor; delete src_accessor;
			} else {
