...
BCView::BCView (BRect rect)
: BView (rect, "brightness/contrast", B_FOLLOW_ALL, B_WILL_DRAW)
{
 	gBrightness = 0;
 	gContrast = 0;
	ResizeTo (Bounds().Width(), 50);
	Slider *bSlid = new Slider (BRect (8, 8, 180, 24), 70, "Brightness", -100, 100, 1, new BMessage ('gblB'));
	Slider *cSlid = new Slider (BRect (8, 28, 180, 44), 70, "Contrast", -100, 100, 1, new BMessage ('gblC'));
	AddChild (bSlid);
	AddChild (cSlid);
...
void BCView::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case 'gblB':
		...
		for (uint32 y = 0; y < h; y++)
		{
			...
			for (uint32 x = 0; x < w; x++)
			{
				...
				if (!inSelection || (map = *(++mapbits)))
				{
					...
					bgra_pixel newpix = PIXEL (clipchar (brightness + contrast*RED(pixel)),
											   clipchar (brightness + contrast*GREEN(pixel)),
											   clipchar (brightness + contrast*BLUE(pixel)),
											   ALPHA(pixel));
