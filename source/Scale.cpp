...
//		firstpoint = point;
		...
		oSelection->Unlock();
		entry = true;
//		float dlt = ((frame->left - firstpoint.x)*(frame->left - firstpoint.x) + (frame->top - firstpoint.y)*(frame->top - firstpoint.y));
//		float drt = ((frame->right - firstpoint.x)*(frame->right - firstpoint.x) + (frame->top - firstpoint.y)*(frame->top - firstpoint.y));
//		float dlb = ((frame->left - firstpoint.x)*(frame->left - firstpoint.x) + (frame->bottom - firstpoint.y)*(frame->bottom - firstpoint.y));
//		float drb = ((frame->right - firstpoint.x)*(frame->right - firstpoint.x) + (frame->bottom - firstpoint.y)*(frame->bottom - firstpoint.y));
//		varcorner = 0; // LeftTop
//		if (drt < dlt) varcorner = 1;	// RightTop
//		if (dlb < drt && dlb < dlt) varcorner = 2;	// LeftBottom
//		if (drb < dlb && drb < drt && drb < dlt) varcorner = 3;	// RightBottom
	...
	if (mode == M_DRAW)
	{
		...
		inLayer->Lock();
		inSelection->Lock();
		CutOrCopy (*outLayer ? *outLayer : inLayer, oLayer, inSelection, int (oBounds.left), int (oBounds.top), true);
		inSelection->Unlock();
		inLayer->Unlock();
		oLayer->Unlock();
	}
...
//	lastpoint = point;
	...
	case M_DRAW:
	{
		...
		// N.B. We also get here when "Apply" is clicked.
		{
			...
			AddWithAlpha (sLayer, (*outLayer) ? (*outLayer) : inLayer, int (newrect.left), int (newrect.top));
			if (*outSelection)
