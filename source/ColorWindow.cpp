	...
	: BWindow(frame, name, B_TITLED_WINDOW, B_NOT_RESIZABLE | B_NOT_ZOOMABLE)
{
	...
	rgbV->AddChild(bRB);
	BString rS, gS, bS;
	fNumberFormat.Format(rS, (int32)c.red);
	fNumberFormat.Format(gS, (int32)c.green);
	fNumberFormat.Format(bS, (int32)c.blue);
...
ColorWindow::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		...
		case 'SetC':
		{
			...
			fNumberFormat.Format(s, (int32)c.red);
			rTC->SetText(s.String());
			fNumberFormat.Format(s, (int32)c.green);
			gTC->SetText(s.String());
			fNumberFormat.Format(s, (int32)c.blue);
		...
		case 'TrbR':
		{
			...
			int r = atoi(rTC->Text());
			r = clipchar(r);
			fNumberFormat.Format(s, (int32)r);
			rTC->SetText(s.String());
			c.red = r;
			rgbSquare->SetColor(c);
			SetColor(c);
		...
		case 'TrbG':
		{
			...
			int g = atoi(gTC->Text());
			g = clipchar(g);
			fNumberFormat.Format(s, (int32)g);
			gTC->SetText(s.String());
			c.green = g;
			rgbSquare->SetColor(c);
			SetColor(c);
		...
		case 'TrbB':
		{
			...
			int b = atoi(bTC->Text());
			b = clipchar(b);
			fNumberFormat.Format(s, (int32)b);
			bTC->SetText(s.String());
			c.blue = b;
			rgbSquare->SetColor(c);
			SetColor(c);
