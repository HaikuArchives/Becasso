...
: AttribView (BRect (0, 0, 164, 90), lstring (26, "Spraycan"))
{
	...
	sSlid = new Slider (BRect (4,  4, 160, 22), 60, lstring (355, "Sigma"), 1, 25, 1, new BMessage ('ASsg'));
	cSlid = new Slider (BRect (4, 26, 160, 44), 60, lstring (356, "Color Ratio"), 0, 1, 0.01, new BMessage ('AScr'), B_HORIZONTAL, 0, "%.2f");
	fSlid = new Slider (BRect (4, 48, 160, 64), 60, lstring (357, "Flow Rate"), 1, 20, 1, new BMessage ('ASfr'));
	AddChild (sSlid);
	AddChild (cSlid);
	AddChild (fSlid);
	fSigma = 5;
	...
	cSlid->SetValue (0);
	fSlid->SetValue (10);
	sL = new BCheckBox (BRect (4, 68, 162, 86), "sL", lstring (358, "Fade with Distance"), new BMessage ('ASfd'));
	sL->SetValue (0);
	fLighten = false;
	AddChild (sL);
	fColorRatio = 0;
...
status_t AttribSpraycan::GetSupportedSuites (BMessage *message)
{
...
BHandler *AttribSpraycan::ResolveSpecifier (BMessage *message, int32 index, BMessage *specifier, int32 command, const char *property)
{
...
void AttribSpraycan::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case B_GET_PROPERTY:
