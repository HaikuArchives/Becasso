...
CanvasWindow::CanvasWindow (BRect frame, const char *name, BBitmap *map, BMessenger *target, bool AskForAlpha, rgb_color color)
		...
		if (!strcmp (type, "image/becasso") || !strcmp (type, "image/x-becasso"))
		{
			...
			*endp = 0;
			if (int (cvers) > int (tvers))	// Check major version
			{
				...
				sprintf (string, lstring (130, "‘%s’ is an image from a newer Becasso version (%s).\nYou can't load these files with this Becasso version."), fName, line);
			...
			if (int (cvers) == int (tvers) && cvers - int (cvers) > tvers - int (tvers))	// Check release
			{
				char string[B_FILE_NAME_LENGTH + 128];
				sprintf (string, lstring (132, "‘%s’ is an image from a newer release of Becasso (%s).\nTry to load anyway?"), fName, line);
				BAlert *alert = new BAlert ("", string, lstring (131, "Cancel"), lstring (133, "Proceed"), NULL, 
					B_WIDTH_AS_USUAL, B_WARNING_ALERT);
				ulong res = alert->Go();
				if (res == 0)
				{
...
//		bms->DisplayProgressBar (BPoint (200, 200), "Reading Image…");
		...
		if (DATATranslate (inStream, NULL, NULL, *bms, DATA_BITMAP))
		{
			...
			BAlert *alert = new BAlert ("", string, "OK", NULL, NULL, 
				B_WIDTH_AS_USUAL, B_WARNING_ALERT);
			alert->Go();
			throw (0);
		}
		...
		if (!err)
		{
			err = BTranslatorRoster::Default()->Translate (&inStream, &info, NULL, bms, B_TRANSLATOR_BITMAP);
			if (!err)
			{
				map = bms->Bitmap();
				bms->SetDispose (false);
		...
		if (err || !map)
		{
			...
			sprintf (errstring, lstring (134, "Translation Kit error:\nCouldn't translate `%s'"), fName);
			BAlert *alert = new BAlert ("", errstring, lstring (135, "Help"), lstring (136, "OK"), NULL, 
				B_WIDTH_AS_USUAL, B_WARNING_ALERT);
...
void CanvasWindow::restOfCtor (BRect canvasFrame, BBitmap *map, FILE *fp, bool AskForAlpha, rgb_color color)
{
	SetPulseRate (50000);
	magWindow = NULL;
	layerOpen = false;
...
//	fileMenu->AddItem (new BMenuItem (lstring (12, "Open File…"), new BMessage ('open'), 'O'));
	...
	fileMenu->AddItem (new BMenuItem (lstring (52, "Export…"), new BMessage ('Expt'), 'S', B_CONTROL_KEY));
	...
	editMenu->AddItem (new BMenuItem (lstring (403, "Paste to New Layer"), new BMessage ('psNL'), 'V', B_SHIFT_KEY));
	editMenu->AddItem (new BMenuItem (lstring (404, "Paste to New Canvas"), new BMessage ('psNC'), 'V', B_CONTROL_KEY));
...
//	canvasFrame.PrintToStream();
	frame.Set (0, 0,
		max_c (canvasFrame.Width() + B_V_SCROLL_BAR_WIDTH, MIN_WIDTH),
...
//	bgFrame.PrintToStream();
	...
	if (map && AskForAlpha)	// AskForAlpha check added here as optimization
	{
		...
		if (avg < 128)
		{
			...
			{
				char alert[2048];
				sprintf (alert, lstring (139, "%s the image `%s' uses alpha as transparency instead of opacity.\n%s inverting the alpha channel."), 
					(avg < 10) ? lstring (140, "I'm pretty sure") : lstring (141, "I suspect"),
					fName,
					(avg < 10) ? lstring (142, "If you're seeing only a checkered pattern,\nI suggest ") : lstring (143, "Either that, or this image has an awful lot of transparent pixels.  If it looks strange, try "));
				BAlert *alphalert = new BAlert ("alphalert", alert, lstring (144, "Keep As-Is"), lstring (145, "Invert"));
...
//				be_app->PostMessage (&invert);
	...
	posFrame.Set (-1, frame.Height() - B_H_SCROLL_BAR_HEIGHT + 1,
		POSWIDTH - 1, frame.Height() + 1);
...
void CanvasWindow::MenusBeginning ()
{
//	make_recent_menu (recentMenu);
	menuIsOn = true;
}

void CanvasWindow::MenusEnded ()
{
	menuIsOn = false;
}


void CanvasWindow::CanvasResized (float /* width */, float /* height */)
{
...
void CanvasWindow::CanvasScaled (float s)
{
	fScale = s;
...
void CanvasWindow::setName (char *s)
{
	strcpy (fName, s);
	char title[MAX_FNAME];
...
void CanvasWindow::setMenuItem (uint32 command, bool v)
{
	BMenuItem *item = editMenu->FindItem(command);
	if (item)
	{
//		char *cmd = (char *)(&command);
//		printf ("Item '%c%c%c%c' %sabled\n", cmd[3], cmd[2], cmd[1], cmd[0], v ? "en" : "dis");
		item->SetEnabled (v);
	}
	else
...
void CanvasWindow::setPaste (bool v)
{
...
void CanvasWindow::FrameResized (float width, float height)
{
...
void CanvasWindow::FrameMoved (BPoint screenpoint)
{
...
void CanvasWindow::ScreenChanged (BRect frame, color_space mode)
{
	Lock();
...
BMessage *CanvasWindow::sendbitmapreply ()
{
...
bool CanvasWindow::QuitRequested ()
{
//	printf ("%s: QuitRequested\n", fName);
	...
	BAlert *alert = new BAlert ("", question, lstring (131, "Cancel"), lstring (147, "Abandon"), lstring (148, "Save"),
...
status_t CanvasWindow::GetSupportedSuites (BMessage *message)
{
...
BHandler *CanvasWindow::ResolveSpecifier (BMessage *message, int32 index, BMessage *specifier, int32 command, const char *property)
{
...
void CanvasWindow::MessageReceived (BMessage *message)
{
	switch (message->what)
	{
		...
		case B_GET_PROPERTY:
		{
			...
			case PROP_ACTIVELAYER:
				if (message->IsSourceWaiting())
				{
					...
					fprintf (stderr, "Current Layer: %d: \"%s\"\n", canvas->currentLayerIndex(), canvas->currentLayer()->getName());
				break;
			default:
				break;
			}
		...
		case B_SET_PROPERTY:
		{
			...
			case PROP_ACTIVELAYER:
			{
				...
				else if (message->FindInt32 ("data", &indexspecifier) == B_OK)
				{
					...
					{
						char errstring[256];
						sprintf (errstring, "Layer Index Out of Range [0..%d]: %ld", canvas->numLayers() - 1, indexspecifier); 
						if (message->IsSourceWaiting())
						{
							BMessage error (B_ERROR);
							error.AddInt32 ("error", B_BAD_SCRIPT_SYNTAX);
...
//					CanvasWindow *canvasWindow = new CanvasWindow (canvasWindowFrame, title, map, &ie);
//					canvasWindow->Show();
			...
			else
			{
				...
				else
				{
					...
					if (entry.Exists())		//// *** NOT TRANSLATED - COPIED FROM OS!!
					{
						char exists[2048];
						BPath path (&entry);
						sprintf (exists, "The file \"%s\" already exists in the specified folder. Do you want to replace it?", path.Leaf());
						button = (new BAlert ("", exists, "Cancel", "Replace", NULL, B_WIDTH_AS_USUAL, B_WARNING_ALERT))->Go();
					}
...
//			delete outputFormatWindow;
		...
		case 'oFsl':
		{
			...
			if (tv)
			{
				...
				// (however, we shouldn't be _exporting_ to Becasso via the TK at all...)
				{
					...
					while (il > 0 && suggestion[il] != '.')	// Yes >0; .dotfiles shouldn't have their entire name cut off...
						il--;
					...
					if (BTranslatorRoster::Default()->GetOutputFormats (out_translator, &format_list, &format_count) == B_NO_ERROR)
					{
						...
						for (int i = 0; i < format_count; ++i)
						{
							...
							if (format_list[i].type == out_type)
							{
								...
								else if (PatronizeMIME)
								{
									...
									sprintf (alertstring,
										lstring (150, 
										"Excuse me for interrupting.  I was trying to suggest an appropriate extension for this file, "\
										"yet couldn't find any in your MIME database for '%s'.\n"\
										"You can add these extensions yourself with the FileTypes preferences application, "\
										"or I would be happy to fill in my suggestions.  You can also simply forget about the "\
										"outdated concept of extensions alltogether, although many other operating systems "\
										"still rely on them for file identification."),
										format_list[i].MIME);
									BAlert *alert = new BAlert (lstring (151, "File Type"), alertstring, lstring (152, "Update MIME"), lstring (153, "Don't Patronize Me"), NULL, B_WIDTH_FROM_WIDEST);
									uint32 button = alert->Go();
									if (button == 1)
									{
										PatronizeMIME = false;
									...
									else	// Update the MIME database
									{
										sprintf (alertstring,
											lstring (154,
											"I have a list of common image file types and their extension but you might have "\
											"some exotic types I don't know about.  In that case, I will try to make an educated "\
											"guess, and if I can't think of anything, I'll simply skip that type.  I will also "\
											"stay away from file types that do have any extensions set.\n"\
											"You can always manually change my suggestions using the FileTypes preferences application."));
										alert = new BAlert (lstring (152, "Update MIME"), alertstring, lstring (131, "Cancel"), lstring (133, "Proceed"));
										button = alert->Go();
										if (button == 1)
										{
											// Do it...
											...
											t = 0;
											while (types.FindString ("types", t++, (const char **) &type) == B_OK)
											{
												// printf ("Working on %s\n", type);
												BMessage mtext;
												BMimeType mt (type);
												...
												|| !mtext.HasString ("extensions"))
												{
													...
													else
													{
														...
														{
															// Maybe we can find a hint in the short description
															// (often something like "PPM Image")
															int spacep;
															for (spacep = 0; spacep < strlen (sd); spacep++)
															{
																if (sd[spacep] == ' ')
																	break;
															}
													...
													if (!foundsomething)	// Well, the last resort.
													{
														...
														// Add more File Types here when we learn about them...
													}
													if (foundsomething)
													{
														// char desc[256];
								...
								if (mime.GetFileExtensions (&extensions) == B_OK)
								{
									...
									// extensions.PrintToStream();
									char *ext;
									if (extensions.FindString ("extensions", 0, (const char **) &ext) == B_OK)
									// Only look for the first one (if there are more)
									{
		...
		case B_SAVE_REQUESTED:
		{
			...
			if (message->FindString ("name"))
			{	// The Save Panel sends the name separately
			...
			if (out_type)	// i.o.w. exporting
			{
				...
				if (roster->Translate (bitmapStream, NULL, NULL, &outStream, out_type))
				{
					...
					if (button == 0)
					{
						alert = new BAlert ("", lstring (156, "This is a rare error.  Something went wrong inside the Translator.\nIt could range from shared library conflicts to running out of hard disk space."), lstring (136, "OK"), NULL, NULL, B_WIDTH_AS_USUAL, B_INFO_ALERT);
						alert->Go();
					}
				}
				#endif
				...
				if (BTranslatorRoster::Default()->GetOutputFormats (out_translator, &format_list, &format_count) == B_NO_ERROR)
				{
					for (int i = 0; i < format_count; ++i)
					{
						if (format_list[i].type == out_type)
						{
							...
							outStream.WriteAttr ("BEOS:TYPE", B_MIME_STRING_TYPE, 0L, mime, strlen (mime) + 1);
							break;
						}
					}
				}
		...
		case 'xpal':
		{
			...
			{
				extractWindow = new XpalWindow (BRect (100, 100, 300, 180), lstring (157, "Extract Palette"), new BMessenger (this));
				extractWindow->Show();
				// extractWindow->Quit();
				// extractWindow = NULL;
			}
...
//			printf ("menubar->Frame().Height() = %f\n", mf.Height());
//			printf ("w = %f, h = %f\n", max_c (MIN_WIDTH, cf.Width() + B_V_SCROLL_BAR_WIDTH), max_c (MIN_HEIGHT, cf.Height() + B_H_SCROLL_BAR_HEIGHT + mf.Height()));
		...
		case 'rszT':	// Resize To
		{
			...
			ResizeWindow *rw = new ResizeWindow (this, title, canvas->Frame().Height() + 1, canvas->Frame().Width() + 1);
			rw->Show();
			break;
		}
		case 'rszt':
...
//				setLayerMenu();
		...
		case B_PASTE:
			if (message->WasDropped())
			{
				...
				canvas->SetScale (1);
				long dummy;
				if (message->FindData ("RGBColor", B_RGB_COLOR_TYPE, (const void **) &dropped, &dummy) == B_OK)
...
//			canvas->Copy();
			...
			if (clip)
			{
				...
				CanvasWindow *canvasWindow = new CanvasWindow (canvasWindowFrame, title, newClip, NULL, false);
