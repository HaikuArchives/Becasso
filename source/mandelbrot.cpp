...
void drawMandel(Layer *layer, uint32 width, uint32 height, bool final, BPoint point,
							Layer *inLayer, Selection *selection, bool julia);

void selectMandel(Selection *outselection, uint32 width, uint32 height, bool final, BPoint point,
...
//***************************************************************************
MandelView::MandelView(BRect rect)
					 :BView (rect, "mandel_view", B_FOLLOW_ALL, B_WILL_DRAW)
//***************************************************************************
{
	mType = K_MANDEL;
	...
	y2 = kh2;
	mIMandel = new BRadioButton(BRect(x2,y2,x4,y2+kh1), "mandel", "Mandelbrot", new BMessage(K_MANDEL_MSG));
	y2 += kh1;
	mIJulia  = new BRadioButton(BRect(x2,y2,x4,y2+kh1), "julia", "Julia", new BMessage(K_JULIA_MSG));
	...
	y2 += kh1;
	mIXCenter = new BTextControl(BRect(x2,y2,x4,y2+kh3), "xcenter", "X Center", "0", new BMessage(K_XCENTER_MSG));
	mIXCenter->SetModificationMessage(new BMessage(K_XCENTER_MSG));
	y2 += kh3;
	mIYCenter = new BTextControl(BRect(x2,y2,x4,y2+kh3), "ycenter", "Y Center", "0", new BMessage(K_YCENTER_MSG));
	...
	y2 += kh1;
	mIXZoom = new BTextControl(BRect(x2,y2,x4,y2+kh3), "xcenter", "X Zoom", "0", new BMessage(K_XZOOM_MSG));
	mIXZoom->SetModificationMessage(new BMessage(K_XZOOM_MSG));
	y2 += kh3;
	mIYZoom = new BTextControl(BRect(x2,y2,x4,y2+kh3), "ycenter", "Y Zoom", "0", new BMessage(K_YZOOM_MSG));
	...
	y2 += kh1;
	mIAngle = new BTextControl(BRect(x2,y2,x4,y2+kh3), "angle", "Angle", "0", new BMessage(K_ANGLE_MSG));
	mIAngle->SetModificationMessage(new BMessage(K_ANGLE_MSG));

	//x5 = max(mXCenter->Divider(),max(mYCenter->Divider(),max(mXZoom->Divider(),max(mYZoom->Divider(),mAngle->Divider()))));
	...
	y2 = kh2;
	mIApplyIter = new BTextControl(BRect(x2,y2,x4,y2+kh3), "applyiter", "Apply Iterations", "0", new BMessage(K_APPLY_ITER_MSG));
	mIApplyIter->SetModificationMessage(new BMessage(K_APPLY_ITER_MSG));
	y2 += kh3;
	mIPreviewIter = new BTextControl(BRect(x2,y2,x4,y2+kh3), "previter", "Preview Iterations", "0", new BMessage(K_PREV_ITER_MSG));
	...
	BMenuField *blockfield = new BMenuField(BRect(x2,y2,x4,y2+kh3), "block", "Preview Block Size", block);
	y2 += kh3;

	mIInvert = new BCheckBox(BRect(x2,y2,x4,y2+kh3), "invert", "Invert Colors", new BMessage(K_INVERT_MSG));
...
//***************************************************************************
void MandelView::MessageReceived (BMessage *msg)
//***************************************************************************
{
	switch (msg->what)
	{
	...
	case K_BLOCK_16_MSG:
		...
		break;
	case K_INVERT_MSG:
		if (mIInvert) mInvert = mIInvert->Value();
		printf("mIInvert %p, value %ld\n", mIInvert, mIInvert->Value());
		break;
	case K_ISOMORPH_MSG:
		if (mIIsomorphic) mIsomorphic = mIIsomorphic->Value();
		break;

	#define M_XY_MSG(msg,obj,var) case msg : if ( obj ) { char *p = (char *) obj ->Text(); if (p) var = atof(p); } break
	...
	#define M_INT_MSG(msg,obj,var) case msg : if ( obj ) { char *p = (char *) obj ->Text(); if (p) var = atol(p); } break
...
char s[64];
	if (LockLooper())
	{
		sprintf(s, "%f", mXCenter);		if (mIXCenter) mIXCenter->SetText(s);
		sprintf(s, "%f", mYCenter);		if (mIYCenter) mIYCenter->SetText(s);
		UnlockLooper();
	}
}
...
//***************************************************************************
void MandelView::displayZoom(void)
//***************************************************************************
{
char s[64];
	if (LockLooper())
	{
		sprintf(s, "%f", mXZoom);		if (mIXZoom) mIXZoom->SetText(s);
		sprintf(s, "%f", mYZoom);		if (mIYZoom) mIYZoom->SetText(s);
		UnlockLooper();
	}
}
...
//***************************************************************************
void MandelView::displayAngle(void)
//***************************************************************************
{
char s[64];
	if (LockLooper())
	{
		sprintf(s, "%ld deg", mAngle);		if (mIAngle) mIAngle->SetText(s);
		UnlockLooper();
	}
}
...
//***************************************************************************
void MandelView::displayIter(void)
//***************************************************************************
{
char s[64];
	if (LockLooper())
	{
		sprintf(s, "%ld", mApplyIterations);	if (mIApplyIter) mIApplyIter->SetText(s);
		sprintf(s, "%ld", mPreviewIterations);	if (mIPreviewIter) mIPreviewIter->SetText(s);
		UnlockLooper();
	}
}
...
int i,j,step,angle;
	...
	if (xzoom < EPSILON) xzoom = EPSILON;
	if (yzoom < EPSILON) yzoom = EPSILON;

	uint32 mods = modifiers();
	if (mods & B_SHIFT_KEY)
	{
		// change zoom value log
		if (view->mLastModifier != B_SHIFT_KEY) view->mLastModifier = B_SHIFT_KEY;
		else
		{
			int fudge=(xzoom>1 ? int (pow(10,log10(xzoom)+1)) : 1);
			xzoom += fudge*(point.x-view->mLastZoom.x)/width;
			if (xzoom < EPSILON) xzoom = EPSILON;
			if (view->isomorphic()) yzoom = xzoom;
			else
			{
	...
	{
		// move center
		if (view->mLastModifier != B_COMMAND_KEY) view->mLastModifier = B_COMMAND_KEY;
		else
		{
			if (julia)
			{
	...
	for(j=height; j>0; j-=step)
	{
		...
		if (julia)	br=((int)(height/2)-j)*yzoom;		// julia
		else				br = ycenter-yzoom*j;					// mandelbrot
		...
		for(i=width; i>0; i-=step)
		{
			if (julia)
			{
				...
				for(k=ki; k>0 && (x2+y2)<4.0; k--)
				{
					...
					// c=(rx,ry) coordinates selected on the mandelbrot set that doesn't move across iterations 
					// z = z2+c
			...
			if (xr > 1)
			{
				...
				for(int yr2=yr; yr2>0; yr2--)
				{
					switch(xr)
					{
						case 16:		*(++dbits2) = pixel;
						case 15:		*(++dbits2) = pixel;
						case 14:		*(++dbits2) = pixel;
						case 13:		*(++dbits2) = pixel;
						case 12:		*(++dbits2) = pixel;
						case 11:		*(++dbits2) = pixel;
						case 10:		*(++dbits2) = pixel;
						case 9:			*(++dbits2) = pixel;
						case 8:			*(++dbits2) = pixel;
						case 7:			*(++dbits2) = pixel;
						case 6:			*(++dbits2) = pixel;
			...
			{
				// step is 1
				if (selection) *(++dbits) = pixelblend (*(++sbits), (pixel | (*(++selbits)) << ALPHA_BPOS));
				else *(++dbits) = pixel | ALPHA_MASK;
			}
		}
	}
	if (final)
...
//		if (selection) selmap += selbpr*step;
		...
		for(i=width; i>0; i-=step)
		{
			...
			if (view->invert()) pixel = 255-pixel;
...
//***************************************************************************
{
	...
	if (!final && buttons) lastpoint = point;
	if (!buttons && view) view->mLastModifier = 0;
