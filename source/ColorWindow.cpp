...
: BWindow (frame, name, B_TITLED_WINDOW, B_NOT_RESIZABLE | B_NOT_ZOOMABLE)
{
	...
	bg2Frame.Set (0, menubarFrame.Height() + CE_HEIGHT - 145, CE_WIDTH, menubarFrame.Height() + CE_HEIGHT);
	...
	aSlid = new Slider (tFrame, 84, lstring (203, "Opacity"), 0, 255, 1, new BMessage ('CXac'), B_HORIZONTAL, 0);
	...
	tFrame.Set (8, 13, 296, 29);
	Slider *tSlid = new Slider (tFrame, 84, lstring (205, "RGB Tolerance"), 0, 255, 1, new BMessage ('CXtc'), B_HORIZONTAL, 0, "%3.1f");
	tSlid->SetValue (button->getTolerance());
	slidBox->AddChild (tSlid);

	setFrame.Set (bg2Frame.Width() - 92, 84, bg2Frame.Width() - 8, 109);
	BButton *setCurrent = new BButton (setFrame, "CE Setbutton", lstring (206, "Current"), new BMessage ('CXSc'));
	bg2->AddChild (setCurrent);
	setCurrent->MakeDefault (true);
	
	setFrame.Set (bg2Frame.Width() - 92, 115, bg2Frame.Width() - 8, 140);
	BButton *setMatch = new BButton (setFrame, "CE Matchbutton", lstring (207, "Best Match"), new BMessage ('CXSm'));
	bg2->AddChild (setMatch);
	
//	cancFrame = setFrame;
//	cancFrame.OffsetBy (-64, 0);
//	BButton *cancel = new BButton (cancFrame, "CE Cancelbutton", "Cancel", new BMessage ('CXcn'));
//	bg2->AddChild (cancel);
	...
	BStringView *matchText = new BStringView (distFrame, "MatchText", lstring (209, "Closest Match in Palette"));
...
void ColorWindow::ScreenChanged (BRect frame, color_space cs)
{
...
void ColorWindow::SetColor (rgb_color _c)
{
	char s[128];
	c = _c;
	cur->SetColor (c);
...
//		char *htmlString;
//		if (msg->FindData ("text/plain", B_MIME_TYPE, (void **) &htmlString, &dummy) == B_OK)
//		{
//			printf ("HTML: <%s>\n", htmlString);
//		}
//		else
