...
bgra_pixel cn_list::most_frequent ()
{
...
public:
			 OilView (BRect rect) : BView (rect, "oil_view", B_FOLLOW_ALL, B_WILL_DRAW)
			 {
			 	gSize = 5;
				ResizeTo (188, 28);
				Slider *sSlid = new Slider (BRect (8, 8, 180, 24), 64, "Mask Size", 3, 15, 2, new BMessage ('oilS'));
				sSlid->SetValue (5);
				AddChild (sSlid);
			 }
virtual		~OilView () {};
...
void OilView::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case 'oilS':
