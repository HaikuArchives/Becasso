...
void
PicMenuButton::AddItem(PicMenu* _m)
{
	menu = _m;
	menu->setParent(this);
}
...
void
PicMenuButton::Draw(BRect update)
{
	...
	SetHighColor(Grey31);
	StrokeLine(update.LeftBottom(), BPoint(update.left, update.top));
...
void
PicMenuButton::MouseDown(BPoint point)
{
	Window()->Lock();
...
void
PicMenuButton::MouseMoved(BPoint point, uint32 transit, const BMessage* msg)
{
	point = point;
	msg = msg;
...
void
PicMenuButton::MessageReceived(BMessage* msg)
{
	//	msg->PrintToStream();
...
int32
PicMenuButton::selected()
{
	return (index);
}

void
PicMenuButton::set(int32 ind)
{
	index = ind;
	menu->Mark(index);
