...
void BitmapView ::Draw(BRect area)
{
	...
	if (IsPrinting() == true)
	{
        ...
        float h_pixel = ( 9.0 / 2.54) * 72.0;
  		...
  		// Centrer l'image
		rect.OffsetTo(0.5*(Bounds().Width() - rect.Width()),0.5*(Bounds().Height() - rect.Height()));
...
void BitmapView::Decode(BPositionIO *stream)
{
	char *bits;
	DATABitmap header;
	stream->Seek(0,SEEK_SET);
	stream->Read(&header, sizeof(header));
	this->ResizeTo(header.bounds.Width(),
					header.bounds.Height());
	Window()->ResizeTo(header.bounds.Width(),
					header.bounds.Height());				
	...
	bitmap=new BBitmap( header.bounds,
						header.colors,
						true);
...
void TestApplication::preRun(void)
{
	status_t res;
	...
	{	
		...
		{				 		
			...
			BWindow *new_bw= new BWindow(area,"Capture", B_TITLED_WINDOW, B_NOT_RESIZABLE | B_NOT_ZOOMABLE);
		
			new_bw->AddChild(view);
			view->Decode(&stream);
			new_bw->Show();
			DATAGetConfigMessage(mw->fHandler,
							&message);
...
MyWindow::MyWindow() :	BWindow(BRect(200,200,400,400), "Capture", B_TITLED_WINDOW, B_NOT_RESIZABLE | B_NOT_ZOOMABLE)
{
	DATAID * list = NULL;
	int32 count = 0;
	int32 ix;
	...
	{
		BAlert *alrt = new BAlert("Not!", "There are no bitmap capture datatypes installed.", "Quit");
		alrt->Go();
	...
	for (ix=0; (ix < count) && DATAMakeCapturePanel( list[ix],
													 new_message,
													 fCapture,
													 x);
													 ix++)
	...
	if (!fCapture)
	{
		BAlert *alrt = new BAlert("Not!", "None of the installed btmap capture datatypes liked me.", "Quit");
		alrt->Go();
		PostMessage(B_QUIT_REQUESTED);
		delete[] list;
		return;
	...
	else
	{
		...
		int32 version;
		fHandler = list[ix];
		DATAGetHandlerInfo(fHandler,
							name,
							info,
							version);
	//	printf("name:%s  info %s version %ld\n",name,info,version);					
		DATAGetConfigMessage(fHandler,
							new_message);
