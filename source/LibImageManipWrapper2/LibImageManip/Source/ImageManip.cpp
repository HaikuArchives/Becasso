...
struct AddonInfo {
	...
	status_t (*Manipulate)(BitmapAccessor* sourceBitmap, BMessage* ioExtension, bool checkOnly);

	status_t (*Convert)(
		BitmapAccessor* sourceBitmap, BitmapAccessor* destBitmap, BMessage* ioExtension,
		bool checkOnly
	);

	status_t (*MakeConfig)(BMessage* ioExtension, BView** configView);
...
Image_Version(
	...
	int32* minVersion
) /* will receive the minimum version */
{
	static char vString[80];
	static char vDate[] = __DATE__;
	if (!vString[0]) {
		sprintf(
			vString, "Image manipulation library, Release %d.%d.%d (%s)",
			IMAGE_LIB_CUR_VERSION / 100, (IMAGE_LIB_CUR_VERSION / 10) % 10,
			IMAGE_LIB_CUR_VERSION % 10, vDate
		);
	}
...
LoadAddonImage(image_addon_id imageAddon)
{
	...
	info->canManipulate =
		get_image_symbol(image, "Manipulate", B_SYMBOL_TYPE_TEXT, (void**)&info->Manipulate) ==
		B_OK;
	...
	if (get_image_symbol(image, "MakeConfig", B_SYMBOL_TYPE_TEXT, (void**)&info->MakeConfig) !=
		B_OK)
		info->MakeConfig = NULL; // not found

	if (get_image_symbol(
			image, "GetConfigMessage", B_SYMBOL_TYPE_TEXT, (void**)&info->GetConfigMessage
		) != B_OK)
...
LoadDir(const char* path, int32& loadErr, int32& nLoaded)
{
	...
	while ((dent = readdir(dir)) != NULL) {
		...
		if (debug)
			printf("stat(%s) = %08lx\n", cwd, err);
		if (!err && S_ISREG(stbuf.st_mode) && strcmp(dent->d_name, ".") &&
			strcmp(dent->d_name, "..")) {
...
) /* will receive number in list */
{
	...
	*outCount = 0;
	...
	while (info) {
		...
		if (info->canManipulate && LoadAddonImage(imageAddon) && info->Manipulate &&
...
) /* will receive number in list */
{
	...
	*outCount = 0;
	...
	while (info) {
		...
		if (info->canConvert && LoadAddonImage(imageAddon) && info->Convert &&
...
status_t
Image_Manipulate(
	image_addon_id imageManipulator, BitmapAccessor* sourceBitmap,
	BMessage* ioExtension, /* can be NULL */
	bool checkOnly
)
{
...
status_t
Image_Convert(
	image_addon_id imageConverter, BitmapAccessor* sourceBitmap,
...
)
{
	...
	return info->Convert(sourceBitmap, destBitmap, ioExtension, checkOnly);
}

/* Let an add-on make a BView that allows the user to configure it */
status_t
Image_MakeConfigurationView(
	image_addon_id imageAddon, BMessage* ioExtension, /* can be NULL */
	BView** configView
) /* will receive pointer to the new BView */
{
	*configView = NULL;
...
Image_GetConfigurationMessage(
	image_addon_id imageAddon, BMessage* ioExtension, /* message to add config info to */
	BMessage* ioCapability
) /* message to add capability info to */
{
