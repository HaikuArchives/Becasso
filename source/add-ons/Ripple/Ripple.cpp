...
bgra_pixel interpolate (float x, float y, bgra_pixel p0, bgra_pixel p1, bgra_pixel p2, bgra_pixel p3);
...
public:
			 RippleView (BRect rect) : BView (rect, "ripple view", B_FOLLOW_ALL, B_WILL_DRAW)
			 {
				...
				gInvDecay = false;
				ResizeTo (188, 216);
				Slider *aSlid = new Slider (BRect (8, 8, 180, 24), 68, "Amplitude", 0, 100, 1, new BMessage ('rplA'));
				Slider *wSlid = new Slider (BRect (8, 28, 180, 44), 68, "Wavelength", 1, 100, 1, new BMessage ('rplW'));
				Slider *pSlid = new Slider (BRect (8, 48, 180, 64), 68, "Phase", 0, 360, 1, new BMessage ('rplP'));
				Slider *dSlid = new Slider (BRect (8, 68, 180, 84), 68, "Decay", 0, 1, 0.01, new BMessage ('rplD'), B_HORIZONTAL, 0, "%.2f");
				Slider *xSlid = new Slider (BRect (8, 88, 180, 104), 68, "X Scale", 0, 2, 0.01, new BMessage ('rplX'), B_HORIZONTAL, 0, "%.2f");
				Slider *ySlid = new Slider (BRect (8, 108, 180, 124), 68, "Y Scale", 0, 2, 0.01, new BMessage ('rplY'), B_HORIZONTAL, 0, "%.2f");
				AddChild (aSlid);
				AddChild (wSlid);
				AddChild (pSlid);
				AddChild (dSlid);
				...
				xSlid->SetValue (1);
				ySlid->SetValue (1);
				iD = new BCheckBox (BRect (8, 130, 180, 154), "invdecay", "Inverse Decay", new BMessage ('invD'));
				AddChild (iD);
				iD->SetValue (false);
				BBox *type = new BBox (BRect (4, 158, 180, 212), "type");
				type->SetLabel ("Type");
				AddChild (type);
				BRadioButton *tA = new BRadioButton (BRect (8, 13, 164, 30), "ang", "Angular", new BMessage ('rptA'));
				BRadioButton *tR = new BRadioButton (BRect (8, 30, 164, 47), "rad", "Radial", new BMessage ('rptR'));
				type->AddChild (tA);
				type->AddChild (tR);
				tR->SetValue (true);
			 }
...
void RippleView::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case 'rplW':
...
bgra_pixel interpolate (float x, float y, bgra_pixel p0, bgra_pixel p1, bgra_pixel p2, bgra_pixel p3)
{
	...
	return (PIXEL(y*ru + (1 - y)*rd, y*gu + (1 - y)*gd, y*bu + (1 - y)*bd, y*au + (1 - y)*ad));
}
...
//			printf ("%d ", row); fflush (stdout);
			...
			for (col = firstCol; col <= lastCol; col++)
			{
				if (!inSelection || *(++ml))
				{
					...
					delta = amplitude*((decay != 0.0) ? (invdecay ? (r/scaleparam)/decay : (1 - decay)*(scaleparam - r)/scaleparam): 1.0)*sin (r/redwavelength + phase);
					if (type == RADIAL)
					{
