...
PosView::PosView (const BRect frame, const char *name, 
	CanvasView *_canvas)
...
void PosView::SetPoint (const int x, const int y)
{
	set_x = x;
	set_y = y;
	if (x == y == -1)
...
void PosView::DoRadius (const bool r)
{
	do_radius = r;
}

void PosView::SetTextLayer (const bool t)
{
	is_textlayer = t;
}

void PosView::Draw (BRect /* updaterect */)
{
	...
	StrokeRect (Bounds());
	SetHighColor (Grey31);
	StrokeLine (Bounds().LeftTop() + BPoint (1, 1),
		Bounds().RightTop() + BPoint (-1, 1));
	...
	if (set_x >= 0)
	{
		...
		if (do_radius)
			sprintf (s, "(%i,%i)∆(%s%i,%s%i) %.1f", mouse_x, mouse_y, delta_x > 0 ? "+" : "", delta_x, delta_y > 0 ? "+" : "", delta_y, sqrt (delta_x*delta_x + delta_y*delta_y));
		else
			sprintf (s, "(%i,%i) ∆(%s%i,%s%i)", mouse_x, mouse_y, delta_x > 0 ? "+" : "", delta_x, delta_y > 0 ? "+" : "", delta_y);
	}
	else
