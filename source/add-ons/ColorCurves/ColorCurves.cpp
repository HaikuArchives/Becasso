...
public:
			 CurveView (BRect rect, const char *name) : BView (rect, name, 0, B_WILL_DRAW)
			 {
			 	...
			 	for (int i = 0; i < NUM_CHANNELS; i++)
			 	{
				 	...
				 	points[i][1].x = 255; points[i][1].y = 255;
				 	FillTable (i);
				 }
				 fCurrent = 99;
			 }
...
int			 Channel ()
			 {
			 	return (cspacePU->IndexOf (cspacePU->FindMarked())*4 + graphPU->IndexOf (graphPU->FindMarked()));
			 }
...
void CurveView::Draw (BRect update)
{
	int chan;
	...
	if (type != GT_GAMMA)
	{
		...
		for (int i = 0; i < npoints[chan]; i++)
			StrokeRect (BRect (points[chan][i].x - 3, 252 - points[chan][i].y, points[chan][i].x + 3, 258 - points[chan][i].y));
	}
}

void CurveView::SetType (int t)
{
	type = t;
	FillTable (fCurrent);
	addon_preview();
...
void CurveView::SetCurrent (int current)
{
...
void CurveView::do_FillTable (int channel)
{
//	printf ("Filling %d\n", channel);
	switch (type)
	{
	case GT_STRAIGHT:
	{
		for (int i = 0; i < npoints[channel] - 1; i++)
		{
			for (int j = points[channel][i].x; j <= points[channel][i + 1].x; j++)
				wct[channel][j] = (unsigned char) (float (points[channel][i + 1].y - points[channel][i].y)*(j - points[channel][i].x)/(points[channel][i + 1].x - points[channel][i].x) + points[channel][i].y);
		}
		break;
	}
	case GT_SMOOTH:
	{
		...
		{
			for (int j = 0; j <= 255; j++)
				wct[channel][j] = (unsigned char) (float (points[channel][1].y - points[channel][0].y)*j/255 + points[channel][0].y);
		}
		else
		{
			float a, b, c, d;
...
//					/ (points[2].x - points[0].x);
			ynprime = float (points[channel][2].y - points[channel][0].y)/(points[channel][2].x - points[channel][0].x);
			a = ynprime/points[channel][1].x - float (points[channel][1].y - points[channel][0].y)/(points[channel][1].x*points[channel][1].x);
			b = 2*float (points[channel][1].y - points[channel][0].y)/points[channel][1].x - ynprime;
			...
			for (int i = 1; i < npoints[channel] - 2; i++)
			{
				...
				float xt = xr - xl;
				yprime = ynprime;
//				ynprime = (float (yr - yl)/(xr - xl)*(points[i + 2].x - points[i + 1].x)
//						 + float (points[i + 2].y - yr)/(points[i + 2].x - points[i + 1].x)*xt)
//						 / (xr - points[i - 1].x);
				ynprime = float (points[channel][i + 2].y - points[channel][i].y)/(points[channel][i + 2].x - points[channel][i].x);
			...
			for (int j = points[channel][npoints[channel] - 2].x; j <= points[channel][npoints[channel] - 1].x; j++)
			{
...
void CurveView::FillTable (int channel)
{
	if (channel == -1)
		channel = fCurrent;
	if (channel == 99)
...
void CurveView::do_AddPoint (int channel)
{
...
void CurveView::MouseDown (BPoint point)
{
	if (fCurrent == 99)
	{
		for (int i = 0; i < npoints[0]; i++)
		{
			BRect handle (points[0][i].x - 3, 252 - points[0][i].y, points[0][i].x + 3, 257 - points[0][i].y);
	...
	else
	{
		...
		{
			BRect handle (points[fCurrent][i].x - 3, 252 - points[fCurrent][i].y, points[fCurrent][i].x + 3, 257 - points[fCurrent][i].y);
			if (handle.Contains (point))
			{
				// Grabbed a handle, start tracking!
				uint32 buttons = 1;
...
void ColorCurvesView::MessageReceived (BMessage *msg)
{
 	switch (msg->what)
 	{
 	case 'cspL':
 	...
 	case 'cspH':
 	{
		...
		graphPU = new BPopUpMenu ("Graphed");
		BMenuItem *item;
		graphPU->AddItem (item = new BMenuItem ("Hue", new BMessage ('HspH')));	// doesn't make sense but looks funny
		item->SetTarget (this);
			...
			for (uint32 x = 0; x < w; x++)
			{
				if (!inSelection || *(++mapbits))
				{
					if (cspace == 0 || cspace == 3)		// RGBA
					{
						...
						*(++dbits) = PIXEL (wct[0][RED (pixel)], wct[1][GREEN (pixel)], wct[2][BLUE (pixel)], wct[3][ALPHA (pixel)]);
					}
					else if (cspace == 1)	// CMYK
					{
						uint32 p = *(++sbits);
						cmyk_pixel pixel = bgra2cmyk (p);
						bgra_pixel bgra = cmyk2bgra (PIXEL (wct[6][YELLOW (pixel)], wct[5][MAGENTA (pixel)], wct[4][CYAN (pixel)], wct[7][BLACK (pixel)]));
