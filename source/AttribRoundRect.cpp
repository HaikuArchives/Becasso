...
AttribRoundRect::AttribRoundRect() : AttribView(BRect(0, 0, 148, 190), lstring(32, "Ovals"))
{
	...
	AddChild(bgTab);
	BView* relTab = new BView(
		BRect(2, TAB_HEIGHT + 4, 126, TAB_HEIGHT + 44), "Rel View", B_FOLLOW_LEFT | B_FOLLOW_TOP,
		B_WILL_DRAW
	);
	BView* absTab = new BView(
		BRect(2, TAB_HEIGHT + 4, 126, TAB_HEIGHT + 44), "Abs View", B_FOLLOW_LEFT | B_FOLLOW_TOP,
		B_WILL_DRAW
	);
	...
	relx = new Slider(
		BRect(4, 2, 142, 18), 10, "x", 0, 0.5, 0.01, new BMessage('RRRx'), B_HORIZONTAL, 0, "%1.2f"
	);
	rely = new Slider(
		BRect(4, 22, 142, 38), 10, "y", 0, 0.5, 0.01, new BMessage('RRRy'), B_HORIZONTAL, 0, "%1.2f"
	);
