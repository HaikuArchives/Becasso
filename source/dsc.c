...
int32 dsc_timer_thread(void *ctrlp)
{
...
int32 dsc_reader_thread(void *ctrlp)
{
	struct reader_s	*ctrl;
	...
	while (ctrl->r < ctrl->count)
		ctrl->r += read(
			ctrl->fd,
			ctrl->buf + ctrl->r, ctrl->count - ctrl->r
		);
...
ssize_t dsc_read(dsc_t *dsc, void *buf, size_t count)
{
#ifdef DEBUGREAD
	int		i;
#endif
	...
	if (count > 0)
	{
		...
		ctrl.target = spawn_thread(
			dsc_reader_thread, "read fd",
			B_NORMAL_PRIORITY, &ctrl
		); 

		timer_id = spawn_thread(
			dsc_timer_thread, "timeout",
			B_NORMAL_PRIORITY, &ctrl
		);
...
# endif
	...
	while (n < count && timerisset(&tv))
	{
		...
		if (s == 1)
		{
			if ((r = read(dsc->fd, (u_int8_t*)buf + n, count - n))
				== -1)
			{
...
# endif
	...
	fprintf(
		stderr,
		__BASE_FILE__
		": dsc_read() timeout (%u/%u) "
		"return from line %u, code == %u\n",
		(unsigned int)n, (unsigned int)count,
		__LINE__ + 4, EDSCRTMOUT
	);
#endif
...
int dsc2_send_cmd(dsc_t *dsc, int cmd, int data, int sequence)
{
	u_int8_t	databuf[16];
	int		n;
...
dsc_t *dsc_open(const char *pathname, speed_t speed, dsc_error *error)
{
	...
	static const u_int8_t
		s_init1[] = { 0x00, 0x00, 0x00, 0x01, 0x04 },
	...
	tcdrain(dsc->fd);
	if (	dsc_read(dsc, dsc->buf, 12 + 9) != 12 + 9 ||
	...
	switch (dsc->buf[20])
	{
		case '1':
			...
			tcdrain(dsc->fd);
			if (	dsc_read(dsc, dsc->buf, 12 + 6) != 12 + 6 ||
				memcmp(dsc->buf, r_prefix, 12) != 0 ||
				memcmp(dsc->buf + 12, r_ok_1, 6) != 0)
				returnerror(EDSCNOANSW, dsc_open);
				/* no answer from camera */
...
int dsc_close(dsc_t *dsc)
{
	...
	static const u_int8_t
		s_reset[] = { 0x00, 0x00, 0x00, 0x00, 0x1f };
...
int dsc_getindex(dsc_t *dsc, dsc_quality_t *buf)
{
#ifdef DEBUG
	int		errorline;
#endif
	...
	static const u_int8_t
		s_index[] = { 0x00, 0x00, 0x00, 0x00, 0x07 },
		r_index_1[] = { 0x00, 0x00, 0x00 },
		r_index_2[] = { 0x08, 0x00, 0xff, 0x08 };
...
int dsc_preview(dsc_t *dsc, int index)
{
	...
	static const u_int8_t
		s_request[] = { 0x00, 0x00, 0x00, 0x01, 0x14 };
...
int dsc_delete(dsc_t *dsc, int index)
{
	...
	static const u_int8_t
		s_request[] = { 0x00, 0x00, 0x00, 0x01, 0x11 };
...
ssize_t dsc_requestimage(dsc_t *dsc, int index)
{
#ifdef DEBUG
	int		errorline;
#endif
	...
	static const u_int8_t
		s_request[] = { 0x00, 0x00, 0x00, 0x01, 0x1a },
	...
	switch (dsc->type)
	{
		case dsc1:
			...
			size =	((u_int32_t)dsc->buf[0] << 16) |
				((u_int32_t)dsc->buf[1] << 8) |
				(u_int32_t)dsc->buf[2];
			break;
		...
		case dsc2:
			...
			if (	dsc_read(dsc, dsc->buf, 16) != 16 ||
				memcmp(dsc->buf, r_request_2, 4) != 0)
				returnerror(EDSCBADNUM, dsc_requestimage);
				/* no answer from camera */

			size =	((u_int32_t)dsc->buf[6] << 16) |
				((u_int32_t)dsc->buf[5] << 8);
			break;
...
ssize_t dsc_readimageblock(dsc_t *dsc, int block, void *buf)
{
#ifdef DEBUG
	int		errorline;
#endif
	...
	static const u_int8_t
		getblock[] = { 0x00, 0x00, 0x00, 0x02, 0x1e, 0x00 };
	...
	switch (dsc->type)
	{
		case dsc1:
			...
			tcdrain(dsc->fd);
			if (	dsc_read(dsc, dsc->buf, 12 + 5) != 12 + 5 ||
				memcmp(dsc->buf, r_prefix, 12) != 0 ||
				dsc->buf[12] != 0 ||
				dsc->buf[13] != 0 ||
				dsc->buf[16] != 0)
			...
			size =	((u_int32_t)dsc->buf[14] << 8) |
				(u_int32_t)dsc->buf[15];
...
const char *dsc_strerror(const dsc_error *lasterror)
{
