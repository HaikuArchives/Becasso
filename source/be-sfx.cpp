...
{
	if (buffer != NULL)
		free(buffer);	/*	since we eat an already allocated malloc() block (say, from FindResource())	*/
}
	...
	int				pan)
{
	...
	if (nPlaying >= fPlaySize)
	{
		...
		memcpy(&fPlayList[found], &fPlayList[found+1], sizeof(PlayingEffect)*(fPlaySize-found-1));
		nPlaying = fPlaySize-1;
		SoundCompleted(evicted, kSoundEvicted);
	}
	/*	finally, we know where we live - set up struct to remember us	*/
	fPlayList[nPlaying].data = effect->GetBuffer(fPlayList[nPlaying].frames, fPlayList[nPlaying].loop);
...
void
EffectsPlayer::SetVolume(
	int				volume)	/*	0->15	*/
{
	fVolume = volume;
	if (fVolume > 15) fVolume = 15;
...
void
EffectsPlayer::SetBackgroundVolume(
	int				bgVolume)
{
	fBGVolume = bgVolume;
	if (fBGVolume > 15) fBGVolume = 15;
	if (fBGVolume < 0) fBGVolume = 0;
}
		...
		for (int iz=0; iz<nPlaying; iz++)
		{
			...
			if (offset >= fPlayList[iz].frames)
			{
				...
				else
				{
					...
					memcpy(&fPlayList[iz], &fPlayList[iz+1], sizeof(PlayingEffect)*(nPlaying-iz-1));
					nPlaying--;
