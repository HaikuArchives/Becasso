...
AddOn::AddOn (BEntry entry)
{
	...
	else if (get_image_symbol (fAddOnID, "addon_make_config", B_SYMBOL_TYPE_TEXT, (void **) &addon_make_config))
	{
		if (VerbAddOns)
			fprintf (stderr, "Problems finding addon_make_config routine in %s\n", path.Path());
		throw (1);
	}
	if (process && get_image_symbol (fAddOnID, "addon_close", B_SYMBOL_TYPE_TEXT, (void **) &addon_close))
	{
		if (VerbAddOns)
	...
	if (get_image_symbol (fAddOnID, "addon_color_changed", B_SYMBOL_TYPE_TEXT, (void **) &addon_color_changed))
...
status_t AddOn::Init (uint32 index)
{
	...
	if (VerbAddOns)
		fprintf (stderr, "%s version %i.%i successfully initialized.\n", fInfo.name, fInfo.version, fInfo.release);
	return 0;
}

void AddOn::ColorChanged ()
{
...
status_t AddOn::Open (BWindow *client, const char */*name*/)
{
//	printf ("AddOn::Open() [%s]\n", fInfo.name);
...
void AddOn::SetTargetOfControlsRecurse (BView *target, BView *view)
{
...
status_t AddOn::MakeConfig (BView **view, BRect rect)
{
	return ((*addon_make_config)(view, rect));
}

status_t AddOn::Close (bool client_quits)
{
//	printf ("AddOn::Close() [%s]\n", fInfo.name);
	...
	if (addon_close)
	{
		...
		if (err)
			fprintf (stderr, "Warning: add-on %s returned 0x%lx from addon_close()\n", fInfo.name, err);
	}
...
status_t AddOn::Process (Layer *inLayer, Selection *inSelection, 
	Layer **outLayer, Selection **outSelection, int32 mode,
	BRect *frame, bool final, BPoint point, uint32 buttons)
{
	return ((*process)(inLayer, inSelection, outLayer, outSelection, mode, frame, final, point, buttons));
}

BBitmap *AddOn::Bitmap (char *title)
{
	return ((*bitmap)(title));
}
...
void addon_update_statusbar (float delta, const char *text, const char *trailingText)
{
	if (!currentAddOnWindow)
	{
...
void addon_reset_statusbar (const char *label, const char *trailingText)
{
	if (!currentAddOnWindow)
	{
