...
#define IMAGE_LIB_CUR_VERSION 110
...
extern "C"
{
	...
	_IMPEXP_IMAGEMANIP const char* Image_Version(
		int32* curVersion, /* will receive the current version */
		int32* minVersion
	); /* will receive the minimum version */

	/* Initialize the library before usage */
	_IMPEXP_IMAGEMANIP status_t Image_Init(const char* loadPath); /* NULL for the default */

	/* Shutdown the library after usage */
	_IMPEXP_IMAGEMANIP status_t Image_Shutdown();

	/* Get all image manipulators that support a given source bitmap
	   and io extension */
	_IMPEXP_IMAGEMANIP status_t Image_GetManipulators(
		...
		int32* outCount
	); /* will receive amount in list */

	/* Get all image converters that support a given source bitmap
	   and io extension */
	_IMPEXP_IMAGEMANIP status_t Image_GetConverters(
	...
	_IMPEXP_IMAGEMANIP status_t Image_GetAddonInfo(
	...
	_IMPEXP_IMAGEMANIP status_t Image_Manipulate(
	...
	_IMPEXP_IMAGEMANIP status_t Image_Convert(
	...
	_IMPEXP_IMAGEMANIP status_t Image_MakeConfigurationView(
	...
	_IMPEXP_IMAGEMANIP status_t Image_GetConfigurationMessage(
