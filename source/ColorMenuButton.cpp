...
: BView (frame, name, B_FOLLOW_LEFT, B_WILL_DRAW)
{
	...
	{
		BScreen screen;
		button = new BBitmap (frame, (screen.ColorSpace() == B_COLOR_8_BIT) ? B_COLOR_8_BIT : B_RGBA32);
		// This is because we can't store 16 bit bitmaps, and 32 bit bitmaps don't look too bad on 16 bit screens.
	}
...
void ColorMenuButton::editorSaysBye ()
{
	editorshowing = false;
}

void ColorMenuButton::MouseDown (BPoint point)
{
	Window()->Lock();
...
void ColorMenuButton::ShowEditor ()
{
...
void ColorMenuButton::MouseMoved (BPoint point, uint32 transit, const BMessage *msg)
{
	point = point;
	msg = msg;
...
void ColorMenuButton::Draw (BRect update)
{
	// printf ("CMB::Draw\n");
...
bool ColorMenuButton::approx (rgb_color a, rgb_color b)
...
float ColorMenuButton::cdiff (rgb_color a)
{
	return (diff (a, color()));
}

rgb_color *ColorMenuButton::palette ()
...
void trim_box (CMB_box *box, uint32 *histogram)
{
	uint32 r, g, b;
...
void ColorMenuButton::extractPalette (Layer *l, int max_col, bool clobber)
{
	...
	for (uint32 p = (l->Bounds().IntegerWidth() + 1)*(l->Bounds().IntegerHeight() + 1); p > 0; p--)
	{
		bgra_pixel pixel = *(++s);
		...
		for (int j = 1; j < max_col; j++)
		{
			...
			if (DebugLevel > 2)
				printf ("Biggest: %ld colors R[%d - %d] G[%d - %d] B[%d - %d]\n",
						biggest->num_colors,
						biggest->rmin, biggest->rmax,
						biggest->gmin, biggest->gmax,
		...
		for (int j = 0; j < max_col; j++)
		{
			if (DebugLevel > 2)
				printf ("Box %i: %ld colors R[%d - %d] G[%d - %d] B[%d - %d]\n",
						...
						boxes[j].rmin, boxes[j].rmax,
						boxes[j].gmin, boxes[j].gmax,
...
int ColorMenuButton::getIndexOfClosest (rgb_color c)
{
...
rgb_color ColorMenuButton::getClosest (rgb_color c)
{
	return (menu->ItemAt(getIndexOfClosest (c))->getColor());
}

void ColorMenuButton::set (int _index, bool via_win)
{
	index = _index;
	menu->Mark (index);
	if (Window())
...
bool ColorMenuButton::set (rgb_color c, bool anyway)
...
void ColorMenuButton::setTolerance (rgb_color _t)
{
	rgb_tolerance = _t;
}

void ColorMenuButton::setTolerance (float _t)
{
	tolerance = _t;
}

void ColorMenuButton::setAlpha (uchar a)
{
	alpha = a;	// Note: The Color Editor gives us opacity (of course!)
}

float ColorMenuButton::getTolerance ()
{
	return tolerance;
}
...
rgb_color ColorMenuButton::color ()
{
...
rgb_color ColorMenuButton::ColorForIndex (int ind)
{
...
void ColorMenuButton::Save (BEntry entry)
{
	FILE *fp;
	char line[81];
...
void ColorMenuButton::Load (BEntry entry)
{
	FILE *fp;
	char line[81];
...
void ColorMenuButton::Generate (ulong w)
{	
	if (w == 'PGdf')
	{
		for (int i = 0; i < 256; i++)
...
void ColorMenuButton::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case B_PASTE:
