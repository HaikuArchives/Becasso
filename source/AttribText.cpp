...
: AttribView (BRect (0, 0, 224, 208), lstring (25, "Text"))
{
	...
	BMenuField *dFamily = new BMenuField (BRect (4, 2, 220, 20), "dFamily", lstring (360, "Family: "), fFamilyPU);
	dFamily->SetDivider (50);
	BMenuField *dStyle = new BMenuField (BRect (4, 24, 220, 40), "dStyle", lstring (361, "Style: "), fStylePU);
	dStyle->SetDivider (50);
	AddChild (dFamily);
	AddChild (dStyle);
	szSlid = new Slider (BRect (7, 50, 220,  66), 66, lstring (362, "Pt Size:"), 4, 144, 1, new BMessage ('fcSz'));
	shSlid = new Slider (BRect (7, 70, 220,  86), 66, lstring (363, "Shear:"), 45, 135, 1, new BMessage ('fcSh'));
	rtSlid = new Slider (BRect (7, 90, 220, 106), 66, lstring (364, "Rotation:"), 0, 360, 1, new BMessage ('fcRt'));
	szSlid->SetValue (12);
	shSlid->SetValue (90);
	AddChild (szSlid);
	AddChild (shSlid);
	AddChild (rtSlid);
	aaCheck = new BCheckBox (BRect (7, 109, 216, 127), "anti-aliasing", lstring (365, "Anti Aliasing"), new BMessage ('fcAA'));
	aaCheck->SetValue (1);
	AddChild (aaCheck);
...
//	demo->SetLowColor (White);
	fText = new BTextView (BRect (8, 132, 216 - B_V_SCROLL_BAR_WIDTH, 202 - B_H_SCROLL_BAR_HEIGHT), "text", BRect (0, 0, 1024, 1), &fFont, 0, B_FOLLOW_NONE, B_WILL_DRAW);
...
void AttribText::AttachedToWindow ()
{
	inherited::AttachedToWindow();
}

status_t AttribText::GetSupportedSuites (BMessage *message)
{
...
BHandler *AttribText::ResolveSpecifier (BMessage *message, int32 index, BMessage *specifier, int32 command, const char *property)
{
...
void AttribText::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case B_GET_PROPERTY:
