...
BlurView::BlurView (BRect rect)
: BView (rect, "blurview", B_FOLLOW_ALL, B_WILL_DRAW)
{
 	gX = 5;
 	gY = 5;
	ResizeTo (Bounds().Width(), 50);
	Slider *xSlid = new Slider (BRect (8, 8, 180, 24), 60, "x size", 1, MAXGBLURSIZE, 2, new BMessage ('gblX'));
	Slider *ySlid = new Slider (BRect (8, 28, 180, 44), 60, "y size", 1, MAXGBLURSIZE, 2, new BMessage ('gblY'));
	AddChild (xSlid);
	AddChild (ySlid);
	xSlid->SetValue (gX);
	ySlid->SetValue (gY);
...
void BlurView::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case 'gblX':
...
//	printf ("\n");
	...
	case M_DRAW:
	{
		...
		for (uint32 y = 0; y < h; y++)
		{
			...
			if (y >= h - sy/2 - 1 && y < h)
			{
				memcpy (rows[GBLURRAD + sy/2] + sx/2, ibits + (2*(h - 1) - (y + sy/2))*ilpr, ilpr*4);
				for (int j = -sx/2; j < 0; j++)
				{
					rows[GBLURRAD + sy/2][sx/2 + j] = rows[GBLURRAD + sy/2][sx/2 - j];
					rows[GBLURRAD + sy/2][w + sx/2 - j - 1] = rows[GBLURRAD + sy/2][w + sx/2 + j - 1];
				}
			}
			else if (y < h)
			{
	...
	case M_SELECT:
	{
		...
		for (uint32 y = 0; y < h; y++)
		{
			...
			if (y >= h - sy/2 - 1 && y < h)
			{
				memcpy (rows[GBLURRAD + sy/2] + sx/2, ibits + (2*(h - 1) - (y + sy/2))*ibpr, ibpr);
				for (int j = -sx/2; j < 0; j++)
				{
					rows[GBLURRAD + sy/2][sx/2 + j] = rows[GBLURRAD + sy/2][sx/2 - j];
					rows[GBLURRAD + sy/2][w + sx/2 - j - 1] = rows[GBLURRAD + sy/2][w + sx/2 + j - 1];
