...
void
BGView::MouseMoved(BPoint point, uint32 transit, const BMessage* msg)
{
	BView* canvas = FindView("Canvas");
	if (canvas) {
		BPoint mp = BPoint(point.x - canvas->Frame().left, point.y - canvas->Frame().top);
...
void
BGView::FrameResized(float width, float height)
{
	BView* canvas = FindView("Canvas");
	BRect cFrame;
	if (canvas) {
