...
WrapperView::WrapperView(BRect rect) : BView(rect, "", B_FOLLOW_NONE, 0)
{
	...
	AddChild(new BButton(
		BRect(205, mTop, 265, mTop + 20), "", "Preview", new BMessage(MSG_FILTER_PREVIEW)
	));
	mTop += 32;
	AddChild(new BCheckBox(
		BRect(5, mTop, 265, mTop + 20), "", APPLY_TO_SELECTION, new BMessage(MSG_APPLY_TO_SELECTION)
	));
	mTop += 22;
	ResizeTo(mWidth, mTop + STATUS_TEXT_HEIGHT);

	mStatus = new BStringView(
		BRect(5, mTop, 265, mTop + 16), "", "Status", B_FOLLOW_LEFT | B_FOLLOW_BOTTOM
	);
	AddChild(mStatus);
...
void
WrapperView::SetupFilters(BMenu* sub_menu)
{
...
#endif
		{
			...
			for (int i = outCount - 1; i >= 0; --i) {
				if (Image_GetAddonInfo(
					...
					if (i == outCount - 1)
						SetupConfigView(outList[i], addonName);
				}
			}
			delete[] outList;
		}
...
void
WrapperView::SetupConfigView(int32 addon, const char* name)
{
	mAddonId = addon;
	RemoveView();
	int width;
...
void
WrapperView::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
...
void
WrapperView::SetStatus(const char* text)
{
...
static void
CopySelection(BBitmap* src, BBitmap* dst, int x, int y)
{
	dst->Lock();
