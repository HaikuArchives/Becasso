...
inline v3 prod (const float a, const v3 &v);
inline v3 minus (const v3 &a, const v3 &b);
...
v3 prod (const float a, const v3 &v)
{
	v3 res;
	res.x = a*v.x;
	res.y = a*v.y;
...
v3 minus (const v3 &a, const v3 &b)
{
	v3 res;
	res.x = a.x - b.x;
	res.y = a.y - b.y;
...
BumpView::BumpView (BRect rect)
: BView (rect, "bumpview", B_FOLLOW_ALL, B_WILL_DRAW)
{
	ResizeTo (188, 190);
	Slider *eSlid = new Slider (BRect (8, 8, 180, 24), 64, "Elevation", 0, 90, 1, new BMessage ('bmpE'));
	Slider *aSlid = new Slider (BRect (8, 28, 180, 44), 64, "Angle", 0, 360, 1, new BMessage ('bmpA'));
	Slider *sSlid = new Slider (BRect (8, 48, 180, 64), 64, "Specular", 0, 100, 1, new BMessage ('bmpS'));
	Slider *rSlid = new Slider (BRect (8, 68, 180, 84), 64, "Reflection", 0, 100, 1, new BMessage ('bmpR'));
	Slider *tSlid = new Slider (BRect (8, 88, 180, 104), 64, "Translucent", 0, 100, 1, new BMessage ('bmpT'));
	AddChild (aSlid);
	AddChild (eSlid);
	AddChild (sSlid);
	AddChild (rSlid);
	...
	AddChild (light);
	BRadioButton *rbW = new BRadioButton (BRect (6, 18, 170, 34), "rbW", "White", new BMessage ('bmlW'));
	BRadioButton *rbF = new BRadioButton (BRect (6, 36, 170, 52), "rbF", "Foreground Color", new BMessage ('bmlF'));
	BRadioButton *rbB = new BRadioButton (BRect (6, 54, 170, 70), "rbB", "Background Color", new BMessage ('bmlB'));
	light->AddChild (rbW);
	light->AddChild (rbF);
	light->AddChild (rbB);
	rbW->SetValue (true);
...
void BumpView::MessageReceived (BMessage *msg)
{
	switch (msg->what)
	{
	case 'bmpE':
	...
	case M_DRAW:
	{
		...
		for (uint32 y = 1; y < h - 1; y++)
		{
			...
			for (uint32 x = 1; x < w - 1; x++)
			{
				...
				double samount_tran = (specular == 0) ? 0 : pow (aspec, (101 - specular)/4)*translucent;
				double samount_spec = (specular == 0 || spec < 0) ? 0 : pow (spec, (101 - specular)/4)*reflection;
				...
				*dbits++ = PIXEL (clipchar (ramount*RED (pixel) + sr),
								  clipchar (gamount*GREEN (pixel) + sg),
								  clipchar (bamount*BLUE (pixel) + sb),
								  ALPHA (pixel));
				mapbits++;
			}
			*dbits++ = *sbits++;
