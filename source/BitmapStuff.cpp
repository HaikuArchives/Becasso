...
void InsertGlobalAlpha (Layer *layer[], int numLayers)		// Watermark all layers by inverting
{
...
BBitmap *entry2bitmap (BEntry entry, bool silent)
{
...
void BlendWithAlpha (BBitmap *src, BBitmap *dest, long x, long y, int /* strength */)
{
	// src: 32bit BBitmap
	// dest: 32bit BBitmap
	// Note: Alpha = opacity!
	...
	for (int j = miny; j < maxy; j++)
	{
		for (int i = minx; i < maxx; i++)
		{
			...
			{
				*dest_data	= ((((((destpixel>>16) & 0xFF)*da + ((srcpixel>>16) & 0xFF)*sa)/ta)<<16) & 0x00FF0000) |
							  ((((((destpixel>> 8) & 0xFF)*da + ((srcpixel>> 8) & 0xFF)*sa)/ta)<< 8) & 0x0000FF00) |
...
void AddWithAlpha (BBitmap *src, BBitmap *dest, long x, long y, int strength)
{
	// src: 32bit BBitmap
	// dest: 32bit BBitmap
	// Note: Alpha = opacity!
...
void AddToSelection (Brush *src, BBitmap *dest, long x, long y, int strength)
{
...
void CutOrCopy (BBitmap *src, BBitmap *dest, BBitmap *sel, long offx, long offy, bool cut)
{
...
extern "C" void mmx_scale_32_h (bgra_pixel *s, bgra_pixel *d, int32 h, int32 sw, int32 dw, int32 dbpr);
extern "C" void mmx_scale_32_v (bgra_pixel *s, bgra_pixel *d, int32 w, int32 sh, int32 dh, int32 dbpr);
...
//        nomenclature of the channels is correct on PPC
{
	...
	if (xscale < 1)		// Shrink horizontally
	{
		...
		if (src)	 // Code for the Layer
		{
			...
			time_t start = clock();
			if (UseMMX && sw <= 1024)	// otherwise, fixed point arithmetic in MMX code will overflow.
			{
#if defined (__INTEL__)
				mmx_scale_32_h ((bgra_pixel *) src->Bits(), (bgra_pixel *) stmp->Bits(), sh, sw, dw, stmp->BytesPerRow());
#endif
			}
			else
			{
	...
	if (yscale < 1)		// Shrink vertically
	{
		...
		if (src)		// Code for Layer
		{
			...
			{
#if defined (__INTEL__)
				mmx_scale_32_v ((bgra_pixel *) stmp->Bits(), (bgra_pixel *) dest->Bits(), dw, sh, dh, dest->BytesPerRow());
#endif
			}
			else
			{
...
void Rotate (SBitmap *s, Layer *d, BPoint o, float rad, bool hiq)
{
	float sr = sin (rad);
	float cr = cos (rad);
	float ix, iy;
	...
	if (hiq && rad != 0)	// do anti-aliasing
	{
		for (int i = 0; i <= bh; i++)
		{
			...
			for (int j = 0; j <= bw; j++)
			{
				...
				float A1 = 0, A2 = 0, A3 = 0, A4 = 0, A5 = 0, A6 = 0, A7 = 0, A8 = 0, A9 = 0;	// Areas
				float w, h;
...
//				float r = sqrt (w*w + h*h);
				...
				red = clipchar (float ((RED(a1)*A1
					...
					+ RED(a8)*A8
					+ RED(a9)*A9)/256));
				green = clipchar (float ((GREEN(a1)*A1
					+ GREEN(a2)*A2
					+ GREEN(a3)*A3
					+ GREEN(a4)*A4
					+ GREEN(a5)*A5
					...
					+ GREEN(a8)*A8
					+ GREEN(a9)*A9)/256));
				blue = clipchar (float ((BLUE(a1)*A1
					+ BLUE(a2)*A2
					+ BLUE(a3)*A3
					+ BLUE(a4)*A4
					+ BLUE(a5)*A5
...
void Rotate (SBitmap *s, Selection *d, BPoint o, float rad, bool hiq)
{
	float sr = sin (rad);
	float cr = cos (rad);
	float ix, iy;
