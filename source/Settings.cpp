...
BPoint get_window_origin (uint32 index)
{
	BPoint origin;
...
void set_window_origin (uint32 index, BPoint origin)
{
	g_settings_lock.Lock();
...
BMenuItem *make_recent_menu ()
{
	...
	BMenuItem *recent = new BMenuItem (BRecentFilesList::NewFileListMenu (lstring (12, "Open…"),
		NULL, NULL, be_app, max_entries, false,
		typelist, 2, NULL), new BMessage ('open'));
	...
	if (find_directory (B_USER_SETTINGS_DIRECTORY, &path) == B_OK)
	{
		...
		else	// we have a list of entry_refs now.
		{
			...
			for (int i = min_c (num_entries, max_entries) - 1; i >= 0; i--)
			{
				...
				msg->AddRef ("ref", &ref);
//				printf ("%ld %s %s\n", entryList[i].mtime, ctime(&entryList[i].mtime), name);
				...
				else
				{
					if (!strcmp (name, "window_origin"))
					{
						...
						if (sscanf (ptr, "%ld:", &index) == 1)
						{
							while (*ptr != ':')
								ptr++;
							ptr++;
							if (sscanf (ptr, "%f,%f", &g_settings.origin[index].x, &g_settings.origin[index].y) != 2)
								fprintf (stderr, "Illegal window origin in Becasso settings: %s\n", ptr);
						}
						else
					...
					else if (!strcmp (name, "recent_entries"))
					{
						...
						if (sscanf (ptr, "%ld", &g_settings.recents) != 1)
							fprintf (stderr, "Illegal # of Recent items in Becasso settings: %s\n", ptr);
					}
					...
					else if (!strcmp (name, "selection_render"))
					{
						...
						if (sscanf (ptr, "%ld", &g_settings.selection_type) != 1)
							fprintf (stderr, "Illegal selection_render in Becasso settings: %s\n", ptr);
					}
					else
					{
