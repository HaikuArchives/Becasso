...
DragView::DrawHorz()
{
	...
	if (goAwayState == 1)
		DrawBitmap(goAwayUp, BPoint(0, 1));
	else
		DrawBitmap(goAwayDown, BPoint(0, 1));
...
void
DragView::MouseDown(BPoint where)
{
	BRect goAwayBounds;

	if (orientation == 0)
		goAwayBounds.Set(3, 3, 16, 14);
	else
		goAwayBounds.Set(3, 3, 16, 14);

	if (thMouseID < 0)
		thMouseID = spawn_thread(MouseThread, "BecassoDVMC", B_DISPLAY_PRIORITY, (void*)this);

	// Set a flag for clicking in the goAway or the menuBar
	if (goAwayBounds.Contains(where))
		mouse_status = _GO_AWAY;
	else
		mouse_status = _DRAG;
