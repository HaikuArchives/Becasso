	  ...
	  fTarget(target)
{
	...
	Slider* nSlid = new Slider(BRect(8, 8, 8 + (bw - 16), 24), 50, lstring(184, "# Colors"), 1, 256,
		1, new BMessage('numC'));
	bg->AddChild(nSlid);
	fNum = 256;
	nSlid->SetValue(256);
	fClobberCB = new BCheckBox(BRect(8, 30, 8 + (bw - 16), 46), "clobber",
		lstring(185, "Clobber Remaining Palette"), new BMessage('clob'));
	fClobber = false;
	fClobberCB->SetValue(false);
	fClobberCB->SetEnabled(false);
	bg->AddChild(fClobberCB);
	BButton* xFG = new BButton(BRect(bw - 2 * XPW_BSIZE - 16, 50, bw - XPW_BSIZE - 16, 74), "fg",
		lstring(70, "Foreground"), new BMessage('xpfg'));
	BButton* xBG = new BButton(BRect(bw - XPW_BSIZE - 8, 50, bw - 8, 74), "bg",
		lstring(71, "Background"), new BMessage('xpbg'));
	bg->AddChild(xFG);
	bg->AddChild(xBG);
	fStatus = 0;
}
...
void
XpalWindow::MessageReceived(BMessage* message)
{
	switch (message->what) {
		case 'numC':
