...
OutputFormatWindow::OutputFormatWindow(
	...
	BInvoker *item_selected,
	BInvoker *window_cancelled,
	BTranslatorRoster *roster)

	: BWindow(
		BRect(100.0, 45.0, 425.0, 322.0),
		"Select Format",
		B_TITLED_WINDOW,
		0)
		...
		case DO_SETUP:
			if (!setup_done)
			{
				...
				OutputFormatView *main_view = new OutputFormatView(
					Bounds(), the_stream, selected_invoker, the_roster);
	...
	, config_view(0)
{
	...
	CaptionView *caption_view = new CaptionView(
		BRect(8.0, 0.0, Bounds().right - 8.0, 0.0), "Output Formats");
...
InfoLinesView::Draw(BRect /*updateRect*/)
{
	for (int32 i = 0; i < mLines; ++i)
	{
		if (mLineTexts[i])
		{
			DrawString(
				mLineTexts[i],
				BPoint(0.0, (i + 1.0) * text_height - text_descent));
		}
	}
}
