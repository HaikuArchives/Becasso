...
void HSVSquare::AttachedToWindow ()
{
	for (int s = 0; s < 256; s++)
		for (int x = 0; x < 32; x++)
		{
...
void HSVSquare::ScreenChanged (BRect /* frame */, color_space cs)
{
...
void HSVSquare::Draw (BRect /* update */)
{
	BRect middle, right;
...
void HSVSquare::MouseDown (BPoint point)
{
	if (point == prev)
		clicks++;
	else
...
void HSVSquare::mouseDown (BPoint point, int ob)
{
...
void HSVSquare::SetColor (rgb_color c)
{
	SetColor (rgb2hsv (c));
}

void HSVSquare::SetColor (hsv_color h)
{
...
rgb_color HSVSquare::GetColor ()
{
	return current;
}

hsv_color HSVSquare::GetColorHSV ()
{
	return currentHSV;
}
