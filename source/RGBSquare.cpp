...
void RGBSquare::AttachedToWindow ()
{
	current = editor->rgb();
	SetColor (current);
...
void RGBSquare::ScreenChanged (BRect /* frame */, color_space mode)
{
...
void RGBSquare::Draw (BRect /* update */)
{
	BRect middle, right;
...
void RGBSquare::MouseDown (BPoint point)
{
	if (point == prev)
		clicks++;
	else
...
void RGBSquare::mouseDown (BPoint point, int ob)
{
	if (ob == 1)
	{
		float prevy = -1;
...
void RGBSquare::SetColor (rgb_color c)
...
void RGBSquare::SetNotColor (int nc)
{
	notcolor = nc;
	SetColor (current);
}

rgb_color RGBSquare::GetColor ()
{
	return current;
}