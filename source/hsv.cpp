...
IMPEXP hsv_color rgb2hsv (rgb_color c)
{
	hsv_color res;
...
IMPEXP rgb_color hsv2rgb (hsv_color c)
{
...
IMPEXP float diff (rgb_color a, rgb_color b)
...
IMPEXP float cdiff (rgb_color a, rgb_color b)
// Note: Doesn't take alpha channel into account.
{
...
IMPEXP uchar clipchar (float x)
{
	return ((x < 0) ? 0 : ((x > 255) ? 255 : uchar (x)));
}

IMPEXP uchar clipchar (int x)
{
	return ((x < 0) ? 0 : ((x > 255) ? 255 : x));
}

IMPEXP float clipone (float x)
{
	return ((x < 0) ? 0 : ((x > 1) ? 1 : x));
}

IMPEXP float clipdegr (float x)
{
	if (x < 0)
...
IMPEXP bgra_pixel cmyk2bgra (cmyk_pixel x)
...
IMPEXP cmyk_pixel bgra2cmyk (bgra_pixel x)
// Alpha is lost
{
	int c, m, y, k;
	y = 255 - BLUE (x);
...
IMPEXP rgb_color bgra2rgb (bgra_pixel c)
{
	rgb_color res;
	res.blue  = BLUE (c);
	res.green = GREEN (c);
...
IMPEXP rgb_color cmyk2rgb (cmyk_pixel c)
{
	return (bgra2rgb (cmyk2bgra (c)));
}

IMPEXP hsv_color bgra2hsv (bgra_pixel c)
{
	return (rgb2hsv (bgra2rgb (c)));
}

IMPEXP hsv_color cmyk2hsv (cmyk_pixel c)
{
	return (rgb2hsv (cmyk2rgb (c)));
}

IMPEXP bgra_pixel rgb2bgra (rgb_color c)
{
#if defined (__POWERPC__)
	return (((c.blue  << 24) & 0xFF000000) | 
			((c.green << 16) & 0x00FF0000) |
			((c.red   <<  8) & 0x0000FF00) | 
			((c.alpha      ) & 0x000000FF));
#else
	return (((c.blue)        & 0x000000FF) | 
			((c.green <<  8) & 0x0000FF00) |
			((c.red   << 16) & 0x00FF0000) | 
			((c.alpha << 24) & 0xFF000000));
#endif
}

IMPEXP bgra_pixel hsv2bgra (hsv_color c)
{
	return (rgb2bgra (hsv2rgb (c)));
}

IMPEXP cmyk_pixel rgb2cmyk (rgb_color c)
{
	return (bgra2cmyk (rgb2bgra (c)));
}

IMPEXP cmyk_pixel hsv2cmyk (hsv_color c)
{
